...
function setupHTMLAndData() {
	...
	for (var c = 0; c < 3; c++) {
		...
		var branchUpgrades = [
			"Gain <span id='" + color + "UpgPow1'></span>x " + color + " quark spins, but " + color + " quarks decay <span id='" + color + "UpgSpeed1'></span>x faster.",
			"The gain of " + color + " <span id='" + color + "UpgName2'></span> quarks is multiplied by x and then raised to the power of x.",
			["Red", "Green", "Blue"][c] + " <span id='" + color + "UpgName3'></span> quarks decay 4x slower.",
		];
		...
		for (var u = 1; u < 4; u++)
			html +=
				...
				color +
				" quark spin</button>" +
				(u == 2 ? "<br><button class='storebtn' style='width: 190px' onclick='maxBranchUpg(\"" + shorthand + "\")'>Max all upgrades</button><br><button class='storebtn' style='width: 190px; font-size:10px' onclick='maxBranchUpg(\"" + shorthand + "\", true)'>Max 2nd and 3rd upgrades</button>" : "") +
				"</td>";
	...
	var row = 0;
	for (var r = 1; r <= 8; r += 2) {
		nfRewards.insertRow(row).innerHTML = "<td id='nfRewardHeader" + r + "' class='milestoneText'></td>" + "<td id='nfRewardHeader" + (r + 1) + "' class='milestoneText'></td>";
		row++;
		nfRewards.insertRow(row).innerHTML = "<td id='nfRewardTier" + r + "' class='milestoneTextSmall'></td>" + "<td id='nfRewardTier" + (r + 1) + "' class='milestoneTextSmall'></td>";
		row++;
		nfRewards.insertRow(row).innerHTML = "<td><button class='nfRewardlocked' id='nfReward" + r + "'></button></td>" + "<td><button class='nfRewardlocked' id='nfReward" + (r + 1) + "'></button></td>";
		row++;
	}
...
function updateChartValues() {
	...
	if (player.options.chart.updateRate <= 200 && player.options.chart.duration >= 30 && player.options.chart.warning === 0) {
		alert("Warning: Setting the duration and update rate to a high value can cause performance issues. Please make sure you know what you're doing before turning the chart on.");
	...
	if ((player.options.chart.duration / player.options.chart.updateRate) * 1000 >= 1000 && player.options.chart.warning !== 2) {
		alert("Warning: You have set the duration and update rate to a very high value. Please make sure you know what you're doing, or have a good computer before turning the chart on.");
...
let tmp = {
	...
	nu: [],
	nuc: [null, 1e6, 1e7, 1e8, 2e8, 5e8, 2e9, 5e9, 7.5e9, 1e10, 7e12, 1e18, 1e55, 1e125, 1e160, 1e280],
...
function updateVioletLightBoostTemp() {
	...
	if (loglighteffect6 > 15e3) loglighteffect6 = 15e3 * Math.pow(loglighteffect6 / 15e3, 0.6);
...
function updateInfiniteTimeTemp() {
	...
	if (tmp.ngp3) {
		...
		if ((!tmp.ngp3l || player.aarexModifications.ngudpV) && player.dilation.active && x > 1e5) x = Math.pow(1e20 * x, 0.2);
...
function showTab(tabName, init) {
	if (tabName == "quantumtab" && !player.masterystudies) {
		alert("Quantum was not fully implemented in NG++, and thus it is impossible to go further in this mod.");
		return;
	}
...
function updateMoney() {
	...
	if (player.currentChallenge == "postc6" || inQC(6)) element2.textContent = "There is " + formatValue(player.options.notation, Decimal.pow(player.matter, 20), 2, 1) + " matter.";
	//TODO
...
function getInfinitiedGain() {
	...
	if (player.achievements.includes("r133") && player.meta) infGain = nM(player.dilation.dilatedTime.pow(0.25).max(1), infGain);
...
function getEternitiesAndDTBoostExp() {
	let exp = 0;
	if (player.dilation.upgrades.includes("ngpp2")) exp += player.aarexModifications.ngudpV ? 0.2 : 0.1;
...
function galaxyReqDisplay() {
	...
	document.getElementById("secondResetLabel").innerHTML =
		...
		(totalTypes > 2 ? " + " + getFullExpansion(Math.round(player.dilation.freeGalaxies)) : "") +
...
function intergalacticDisplay() {
	...
	if (player.achievements.includes("ng3p37") && shiftRequirement.tier > 7) {
		...
		document.getElementById("intergalacticLabel").innerHTML =
			...
			" (" +
			getFullExpansion(player.galaxies) +
			(Math.floor(tmp.igg - player.galaxies) > 0 ? " + " + getFullExpansion(Math.floor(tmp.igg - player.galaxies)) : "") +
			"): " +
			shorten(dilates(tmp.ig).pow(player.dilation.active ? getNanofieldRewardEffect(5, "dil_exp") : 1)) +
...
function ng3p51Display() {
	if (!player.achievements.includes("ng3p51")) document.getElementById("bigRipStatistics").style.display = "none";
		...
		setAndMaybeShow("bestmoneythisrip", tmp.qu.bigRip.active, "'Your best antimatter in this Big Rip is ' + shortenMoney(tmp.qu.bigRip.bestThisRun) + '.'");
...
function infinityUpgradesDisplay() {
	...
	document.getElementById("infi11").innerHTML = "Normal Dimensions gain a multiplier based on time played in this save file<br>Currently: " + infUpg11Pow().toFixed(2) + "x<br>Cost: 1 IP";
...
function preBreakUpgradeDisplay() {
	...
	document.getElementById("infiMult").innerHTML = "You get 2x more IP. " + Math.round(getIPMultPower() * 100) / 100 + infiMultEnding;
	...
	if (player.infinityUpgradesRespecced != undefined) {
		...
		if (player.galacticSacrifice) {
			...
			document.getElementById("infi21").innerHTML = "Increase the multiplier for buying 10 Dimensions based on your Infinities<br>" + base + "x -> " + (infUpg12Pow() * base).toPrecision(4) + "x<br>Cost: 1 IP";
			document.getElementById("infi33").innerHTML = "Dimension boosts are stronger based on IP<br>Currently: " + (1.2 + 0.05 * player.infinityPoints.max(1).log(10)).toFixed(2) + "x<br>Cost: 7 IP";
		}
		...
		document.getElementById("infi34").innerHTML = "You generate IP based on your fastest Infinity time " + infi34Middle + "<br>Cost: 10 IP";
	}
...
function infoScaleDisplay() {
	if (player.aarexModifications.hideRepresentation) document.getElementById("infoScale").textContent = "";
		...
		if (years >= 1e12) var message = "<br>If you end the non-stop writing of your full antimatter amount with 3 digits per second, you would start it " + (years / 1e12).toFixed(2) + " TYA.";
			...
			var since;
			var sinceYears;
			var dates = [5332e3, 35e5, 258e4, 781e3, 315e3, 25e4, 195e3, 16e4, 125e3, 7e4, 67e3, 5e4, 45e3, 4e4, 35e3, 33e3, 31e3, 29e3, 28e3, 2e4, 16e3, 15e3, 14e3, 11600, 1e4, 8e3, 6e3, 5e3, 4e3, 3200, 3000, 2600, 2500, 2300, 1800, 1400, 1175, 800, 753, 653, 539, 356, 200, 4, 0];
			var events = [
...
function breakInfinityUpgradeDisplay() {
	...
	document.getElementById("postinfi11").innerHTML = "Power up Normal Dimensions based on total antimatter produced<br>Currently: " + shorten(Math.pow(player.totalmoney.e + 1, player.galacticSacrifice ? 2 : 0.5)) + "x<br>Cost: " + shortenCosts(1e4) + " IP";
	document.getElementById("postinfi21").innerHTML = "Power up Normal Dimensions based on your current antimatter<br>Currently: " + shorten(Math.pow(player.money.e + 1, player.galacticSacrifice ? 2 : 0.5)) + "x<br>Cost: " + shortenCosts(5e4) + " IP";
	...
	document.getElementById("postinfi22").innerHTML = "Power up Normal Dimensions based on achievements completed <br>Currently: " + shorten(achievementMult) + "x<br>Cost: " + shortenCosts(1e6) + " IP";
	document.getElementById("postinfi12").innerHTML = "Power up Normal Dimensions based on your infinities<br>Currently: " + shorten(getInfinitiedMult()) + "x<br>Cost: " + shortenCosts(1e5) + " IP";
	document.getElementById("postinfi41").innerHTML = "Make galaxies " + Math.round(getPostGalaxyEff() * 100 - 100) + "% stronger <br>Cost: " + shortenCosts(5e11) + " IP";
	document.getElementById("postinfi32").innerHTML = "Power up Normal Dimensions based on your slowest normal challenge run time<br>Currently: " + shorten(worstChallengeBonus) + "x<br>Cost: " + shortenCosts(1e7) + " IP";
	document.getElementById("postinfi13").innerHTML = "You passively generate Infnities based on your fastest infinity.<br>1 Infinity every " + timeDisplay(player.bestInfinityTime * 5) + " <br>Cost: " + shortenCosts(2e7) + " IP";
...
function eternityUpgradesDisplay() {
	...
	document.getElementById("eter3").innerHTML = "Infinity Dimension multiplier based on " + (player.boughtDims ? "time shards (x/" + shortenCosts(1e12) + "+1)" : "sum of Infinity Challenge times") + "<br>Currently: " + shortenMoney(getEU3Mult()) + "x<br>Cost: " + shortenCosts(5e4) + " EP";
...
function mainDilationDisplay() {
	if (player.dilation.active) uponDilationDisplay();
		...
		document.getElementById("enabledilation").textContent =
			"Dilate time." + ((player.eternityBuyer.isOn && player.eternityBuyer.dilationMode && !player.eternityBuyer.slowStopped && player.eternityBuyer.dilMode == "amount" ? !isNaN(player.eternityBuyer.statBeforeDilation) : false) ? " " + (player.eternityBuyer.dilationPerAmount - player.eternityBuyer.statBeforeDilation) + " left before dilation." : "");
...
function replicantiDisplay() {
	if (player.replicanti.unl) {
		...
		document.getElementById("replicantireset").innerHTML =
			(!tmp.ngp3l && player.achievements.includes("ng3p67") ? "Get " : player.achievements.includes("ngpp16") ? "Divide replicanti amount by " + shorten(Number.MAX_VALUE) + ", but get " : "Reset replicanti amount, but get ") +
			"1 free galaxy.<br>" +
		...
		document.getElementById("replicantiapprox").innerHTML =
			tmp.ngp3 && player.dilation.upgrades.includes("ngpp1") && player.timestudy.studies.includes(192) && player.replicanti.amount.gte(Number.MAX_VALUE) && (!player.aarexModifications.nguspV || player.aarexModifications.nguepV)
				? "Replicanti increases by " +
				  (tmp.rep.est < Math.log10(2) ? "x2.00 per " + timeDisplayShort((Math.log10(2) / tmp.rep.est) * 10) : (tmp.rep.est.gte(1e4) ? shorten(tmp.rep.est) + " OoMs" : "x" + shorten(Decimal.pow(10, tmp.rep.est.toNumber()))) + " per second") +
				  ".<br>" +
				  ...
				  " OoMs.<br>" +
				  "(2x slower per " +
				  getFullExpansion(Math.floor((tmp.rep.speeds.exp * Math.log10(2)) / Math.log10(tmp.rep.speeds.inc))) +
				  " OoMs)"
...
function initialTimeStudyDisplay() {
	...
	document.getElementById("192desc").textContent = "You can have beyond " + shortenMoney(Number.MAX_VALUE) + " replicantis, but the interval is increased the more you have";
...
function getReplMult(next) {
	let exp = 2;
	if (player.galacticSacrifice !== undefined) exp = Math.max(2, Math.pow(player.galaxies, 0.4));
	if (player.boughtDims) {
...
function getIPMult() {
	...
	if (player.galacticSacrifice && player.tickspeedBoosts == undefined) {
		...
		if (player.achievements.includes("r41")) mult = mult.times(Math.pow(Math.log10(Math.max(player.spreadingCancer, 10)), 0.05));
...
function gainedEternityPoints() {
	var ret = Decimal.pow(5, player.infinityPoints.plus(gainedInfinityPoints()).e / (player.achievements.includes("ng3p23") ? 307.8 : 308) - 0.7)
		.times(player.epmult)
		.times(kongEPMult);
	...
	if (player.timestudy.studies.includes(121)) ret = ret.times((253 - averageEp.dividedBy(player.epmult).dividedBy(10).min(248).max(3)) / 5);
...
function setAchieveTooltip() {
	...
	doubleGal.setAttribute(
		...
		"Buy 2 Antimatter Galaxies. " +
			(player.tickspeedBoosts !== undefined ? "Reward: Upon a Tickspeed Boost, your Dimension Boosts" + (player.aarexModifications.ngmX > 3 ? " and Time Dimension Boosts" : "") + " don’t reset unless you have more Tickspeed Boosts than five times your Antimatter Galaxies minus eight." : "") +
			(player.aarexModifications.ngmX > 3 ? " You start with 3 Time Dimension Boosts." : "")
	);
	...
	twomillion.setAttribute("ach-tooltip", "Infinity 2000000 times. Reward: Infinities that are more than 5 seconds long give 250 more infinities" + (player.galacticSacrifice ? ", and you gain an additive +249 infinites per crunch after multipliers" : "") + ".");
	...
	wasted.setAttribute("ach-tooltip", "Get " + shorten(11e6) + " TT without having TT generation, keeping your previous TT, and respeccing studies. Reward: While you have less than 1 hour worth of TT production, you generate 10x as much TT.");
	...
	//ACHIEVEMENT ROW 20
	finite.setAttribute("ach-tooltip", "Get " + shortenCosts(1e33) + " Space Shards without Breaking Eternity in this Ghostify." + (tmp.ngp3l ? "" : " Reward: Outside of Big Rips, Tree Upgrades are 10% stronger. In Big Rips, 8th Time Dimensions gain an small exponent boost based on your current Ghostify time."));
	...
	bm10.setAttribute("ach-tooltip", "Reward: Start Ghostifies with 10 of each Emperor Dimension, up to the 4th tier" + (player.aarexModifications.ngudpV ? ", and start Big Rips with the 3rd row of Eternity upgrades." : "."));
	...
	//ACHIEVEMENT ROW 21
	uc.setAttribute("ach-tooltip", "Become a ghost with at least " + shortenCosts(Decimal.pow(10, 2.2e5)) + " EP without starting Eternity Challenge 10 while Big Ripped." + (tmp.ngp3l ? "" : " Reward: Meta-Dimension Boosts don't reset Meta Dimensions."));
	...
	arent.setAttribute("ach-tooltip", "Reach " + shortenCosts(Decimal.pow(10, 1.8e6)) + " IP while dilated and Big Ripped and without having studies, EP mult upgrades, Tree Upgrades, and Break Eternity in this Ghostify." + (tmp.ngp3l ? "" : " Reward: Your 8th Tree Upgrade's level speeds up Nanofield."));
	...
	igu.setAttribute("ach-tooltip", "Reach " + shortenCosts(Decimal.pow(10, 2.25e4)) + " IP while dilated and Big Ripped with Anti-Dilation modifier and without having studies, EP mult upgrades, Tree Upgrades, and Break Eternity in this Ghostify.");
...
function calcSacrificeBoost() {
	...
	if (player.challenges.includes("postc2") || (player.tickspeedBoosts !== undefined && player.currentChallenge == "postc2")) {
		...
		ret = player.firstAmount
			.pow(0.05)
			.dividedBy(player.sacrificed.pow(player.aarexModifications.ngmX > 3 ? 0.05 : 0.04).max(1))
			.max(1);
...
function buyDilationUpgrade(pos, max, isId) {
	...
	if (rebuyable) {
		...
		if (id == "ngpp6" && tmp.ngp3) {
			...
			if (!quantumed) {
				$.notify("Congratulations, you unlocked Mastery Studies! You can now click the 'mastery studies' button\nor the 'continue to mastery studies' button in the time studies menu to access it.");
				document.getElementById("welcomeMessage").innerHTML = "Congratulations for reaching the end-game of NG++. In NG+3, the game continues with a lot of new content starting at Mastery Studies. You can now either click the 'mastery studies' button or the 'continue to mastery studies' button in time studies to access it, which reveals a lot more time studies.";
			}
		}
	}
	if (max) return true;
...
function gainDilationGalaxies() {
	if (player.dilation.dilatedTime.gte(player.dilation.nextThreshold)) {
		...
		if (player.exdilation != undefined) thresholdMult -= Math.min(0.1 * exDilationUpgradeStrength(2), 0.2);
	...
	if (player.ghostify.milestones > notifyId2) {
		$.notify(
			"You became a ghost in at most " +
				...
				" quantums! " +
				[
					"You now start with all Speedrun Milestones and all " + shorten(Number.MAX_VALUE) + " QK assignation features unlocked, all Paired Challenges completed, all Big Rip upgrades bought, Nanofield is 2x faster until you reach 16 rewards, and you get quarks based on your best MA this quantum",
					...
					"You now start with 10 of each Emperor Dimension, up to the 2nd tier, " + (player.aarexModifications.ngudpV ? " and from now on, you start Big Rips with the 3rd row of Eternity upgrades" : ""),
					...
					"You now start with " + shortenCosts(1e25) + " quark spins and Branches are 10x faster",
...
function doNGP4UnlockStuff() {
	$.notify("Congratulations! For unlocking Big Rip in a NG+3 save, you unlocked NG+4! Check it out in the mods menu.", "success");
	metaSave.ngp4 = true;
	checkForExpertMode();
	localStorage.setItem(metaSaveId, btoa(JSON.stringify(metaSave)));
}
...
function doNGP3UnlockStuff() {
	...
	if (!player.ghostify.reached && tmp.qu.bigRip.active)
		if (tmp.qu.bigRip.bestThisRun.gte(Decimal.pow(10, getQCGoal(undefined, true)))) {
...
function doPerSecondNGP3Stuff() {
	...
	if (isAutoGhostActive(9) && tmp.qu.replicants.quantumFood > 0)
		for (d = 1; d < 9; d++)
			if (canFeedReplicant(d) && (d == tmp.qu.replicants.limitDim || (!tmp.eds[d + 1].perm && tmp.eds[d].workers.lt(11)))) {
...
function nanofieldProducingChargeUpdating(diff) {
	...
	if (toSub.eq(0)) {
		...
		document.getElementById("produceQuarkCharge").innerHTML = "Start producing preon charge.<br>(Preon production is disabled while producing preon charge)";
	} else {
	...
	if (toAddAE.gt(0)) {
		...
		if (tmp.qu.nanofield.power > tmp.qu.nanofield.rewards) {
			...
			if (!tmp.qu.nanofield.apgWoke && tmp.qu.nanofield.rewards >= tmp.apgw) {
				...
				$.notify("You reached " + getFullExpansion(tmp.apgw) + " rewards... The Anti-Preontius has woken up and has taken over the Nanoverse! Be careful!");
				showTab("quantumtab");
...
function getReplicantiFinalInterval() {
	...
	if (player.replicanti.amount.gt(Number.MAX_VALUE)) x = player.boughtDims ? Math.pow(player.achievements.includes("r107") ? Math.max(player.replicanti.amount.log(2) / 1024, 1) : 1, -0.25) * x.toNumber() : Decimal.pow(tmp.rep.speeds.inc, Math.max(player.replicanti.amount.log10() - tmp.rep.speeds.exp, 0) / tmp.rep.speeds.exp).times(x);
	return x;
}
...
function continuousReplicantiUpdating(diff) {
	if (player.timestudy.studies.includes(192) && tmp.rep.est.toNumber() > 0 && tmp.rep.est.toNumber() < 1 / 0) player.replicanti.amount = Decimal.pow(Math.E, tmp.rep.ln + Math.log(((diff * tmp.rep.est) / 10) * (Math.log10(tmp.rep.speeds.inc) / tmp.rep.speeds.exp) + 1) / (Math.log10(tmp.rep.speeds.inc) / tmp.rep.speeds.exp));
		...
		player.replicanti.amount = Decimal.pow(
			...
			tmp.rep.ln +
				tmp.rep.est
					.times((diff * Math.log10(tmp.rep.speeds.inc)) / tmp.rep.speeds.exp / 10)
					.add(1)
					.log(Math.E) /
...
function doEternityButtonDisplayUpdating(diff) {
	...
	if (document.getElementById("eternitybtn").style.display == "inline-block") {
		document.getElementById("eternitybtnFlavor").textContent =
			(!player.dilation.active && gainedEternityPoints().lt(1e6)) || player.eternities < 1 || player.currentEternityChall !== "" || (player.options.theme == "Aarex's Modifications" && player.options.notation != "Morse code") ? (player.currentEternityChall !== "" ? "Other challenges await..." : player.eternities > 0 ? "" : "Other times await...") + " I need to become Eternal." : "";
		if (player.dilation.active && player.dilation.totalTachyonParticles.gte(getDilGain())) document.getElementById("eternitybtnEPGain").innerHTML = "Reach " + shortenMoney(getReqForTPGain()) + " antimatter to gain more Tachyon Particles.";
			...
			if ((EPminpeak.lt(Decimal.pow(10, 9)) && EPminpeakType == "logarithm") || (EPminpeakType == "normal" && EPminpeak.lt(Decimal.pow(10, 1e9)))) {
				document.getElementById("eternitybtnEPGain").innerHTML =
					player.eternities > 0 && (player.currentEternityChall == "" || player.options.theme == "Aarex's Modifications")
...
function doQuantumButtonDisplayUpdating(diff) {
	...
	document.getElementById("quantumbtnFlavor").textContent =
		((tmp.qu !== undefined ? !tmp.qu.times && (player.ghostify !== undefined ? !player.ghostify.milestones : true) : false) || !inQC(0) ? ((tmp.ngp3 ? tmp.qu.bigRip.active : false) ? "I am" : inQC(0) ? "My computer is" : tmp.qu.challenge.length > 1 ? "Paired challenges are" : "My challenging skills are") + " not powerful enough... " : "") + "I need to go quantum.";
...
function currentChallengeProgress() {
	...
	document.getElementById("progresspercent").setAttribute("ach-tooltip", "Percentage to the challenge goal");
}
...
function infDimProgress() {
	...
	document.getElementById("progressbar").style.width = p;
	document.getElementById("progresspercent").textContent = p;
	document.getElementById("progresspercent").setAttribute("ach-tooltip", "Percentage to the next dimension unlock");
}
...
function currentEChallengeProgress() {
	...
	document.getElementById("progressbar").style.width = p;
	document.getElementById("progresspercent").textContent = p;
	document.getElementById("progresspercent").setAttribute("ach-tooltip", "Percentage to the Eternity Challenge goal");
}
...
function d8SacDisplay() {
	document.getElementById("sacrifice").setAttribute("ach-tooltip", "Boosts the 8th Dimension by " + formatValue(player.options.notation, calcSacrificeBoost(), 2, 2) + "x");
...
function galSacBtnUpdating() {
	...
	if (document.getElementById("gSacrifice").style.display === "inline-block") {
		...
		if (getGSAmount().gt(0)) {
			...
			document.getElementById("sacrificebtn").innerHTML = (notationOkay ? "Gain " : "Galactic Sacrifice for ") + shortenDimensions(getGSAmount()) + " Galaxy points." + (notation2okay ? "" : "<br>" + shortenMoney(currentGPmin) + " GP/min" + "<br>Peaked at " + shortenMoney(GPminpeak) + " GP/min");
...
function simulateTime(seconds, real, id) {
	...
	if (player.infinitied > playerStart.infinitied) popupString += "<br>you got " + getFullExpansion(player.infinitied - playerStart.infinitied) + " infinities.";
	if (player.eternities > playerStart.eternities) popupString += " <br>you got " + getFullExpansion(player.eternities - playerStart.eternities) + " eternities.";
...
function switchDecimalMode() {
	if (confirm("You will change this option to " + (player.aarexModifications.breakInfinity ? "logarithmica_numerus_lite" : "break_infinity.min") + ".js. Switching number libraries requires a page reload. Are you sure you want to do that?")) {
		player.aarexModifications.breakInfinity = !player.aarexModifications.breakInfinity;
		if (player.aarexModifications.breakInfinity && !player.aarexModifications.performanceTicks && confirm("WARNING: Your game may lag when using break_infinity.js. Do you want to turn on Performance Ticks? You can change your preference at anytime in the Options menu.")) player.aarexModifications.performanceTicks = true;
		save_game(true);
		document.location.reload(true);
	}
}
