...
function setupHTMLAndData() {
  ...
  document.getElementById("infi14").innerHTML =
    "Decrease the number of Dimensions needed for Dimensional Boosts and Galaxies by 9<br>Cost: 1 IP";
  ...
  for (var c = 0; c < 3; c++) {
    ...
    var branchUpgrades = [
      ...
      ["Red", "Green", "Blue"][c] +
        " <span id='" +
        color +
        "UpgName3'></span> quarks decay 4x slower.",
    ];
    ...
    for (var u = 1; u < 4; u++)
      html +=
        ...
        u +
        ")'" +
        (u < 3 ? " style='font-size:10px'" : "") +
        ">" +
        branchUpgrades[u - 1] +
        "<br>Currently: <span id='" +
        color +
        "upg" +
        u +
        "current'>1</span>x<br>Cost: <span id='" +
        color +
        "upg" +
        u +
        "cost'>?</span> " +
        ...
        (u == 2
          ? "<br><button class='storebtn' style='width: 190px' onclick='maxBranchUpg(\"" +
            shorthand +
            "\")'>Max all upgrades</button><br><button class='storebtn' style='width: 190px; font-size:10px' onclick='maxBranchUpg(\"" +
            shorthand +
    ...
    html +=
      ...
      shorthand +
      "RadioactiveDecay' style='font-size:10px' onclick='radioactiveDecay(\"" +
      shorthand +
      "\")'>Reset to make 1st upgrades stronger, but nerf this branch.<br><span id='" +
      shorthand +
...
function updateChartValues() {
  ...
  if (
    ...
    alert(
      "Warning: Setting the duration and update rate to a high value can cause performance issues. Please make sure you know what you're doing before turning the chart on."
    );
  ...
  if (
    ...
    alert(
      "Warning: You have set the duration and update rate to a very high value. Please make sure you know what you're doing, or have a good computer before turning the chart on."
    );
    player.options.chart.warning = 2;
  }
}
...
let tmp = {
  ...
  lt: [12800, 1.6e5, 4.8e5, 1.6e6, 6e6, 5e7, 2.4e8, 1.25e9],
...
function updateRedLightBoostTemp() {
  var light0multiplier = tmp.newNGP3E ? 0.155 : 0.15;
  var lighteffect0 = Math.pow(tmp.effL[0].best, 1 / 4) * light0multiplier + 1;

  if (lighteffect0 > 1.5 && !tmp.newNGP3E)
    lighteffect0 = Math.log10((lighteffect0 * 20) / 3) * 1.5;
...
function updateOrangeLightBoostTemp() {
  tmp.le[1] =
    tmp.effL[1] > 64
      ? Math.log10(tmp.effL[1] / 64) + 14
      : tmp.effL[1] > 8
      ? Math.sqrt(tmp.effL[1]) + 6
      : tmp.effL[1] + 1;
  tmp.ppti /= tmp.le[1];
}
...
function updateGreenLightBoostTemp() {
  ...
  if (tmp.ngp3l)
    lighteffect3 =
      tmp.effL[3] > 8
        ? Math.log10(tmp.effL[3] / 8) + Math.sqrt(12) + 1
        : Math.sqrt(tmp.effL[3] * 1.5) + 1;
...
function updateIndigoLightBoostTemp() {
  var loglighteffect5 =
    tmp.effL[5] > 25 ? Math.sqrt(tmp.effL[5] * 10 + 375) : tmp.effL[5];
  loglighteffect5 *= tmp.newNGP3E ? 20 : 10;
  if (!tmp.ngp3l && loglighteffect5 > 729)
    loglighteffect5 = Math.pow(loglighteffect5 * 27, 2 / 3);
  tmp.le[5] = Decimal.pow(10, loglighteffect5);
}

function updateVioletLightBoostTemp() {
  var lightexp6 = tmp.newNGP3E ? 0.36 : 1 / 3;
  var loglighteffect6 =
    Math.pow(player.postC3Reward.log10() * tmp.effL[6], lightexp6) * 2;
  if (loglighteffect6 > 15e3)
    loglighteffect6 = 15e3 * Math.pow(loglighteffect6 / 15e3, 0.6);
  if (!tmp.ngp3l && loglighteffect6 > 5e4)
    loglighteffect6 = Math.sqrt(loglighteffect6 * 5e4);
...
function updateEffectiveLightAmountsTemp() {
  ...
  for (var c = 7; c >= 0; c--) {
    ...
    if ((c == 6 && !isLEBoostUnlocked(4)) || c == 7) y += 1;
      ...
      y += Math.pow(
        tmp.effL[c + 1] * tmp.leBonus[5].mult + 1,
        tmp.leBonus[5].exp
      );
...
function updateIntergalacticTemp() {
  ...
  if (
    tmp.be &&
    player.dilation.active &&
    tmp.qu.breakEternity.upgrades.includes(10)
  )
  ...
  if (!tmp.ngp3l && igLog > 1e20) {
    //Dark Matter / Ghostly / Ethereal
...
function updateReplicantiTemp() {
  ...
  if (tmp.ngp3 && player.masterystudies.includes("t273")) {
    ...
    data.freq = 0;
    if (data.chance.gte("1e9999998"))
      data.freq = tmp.mts[273].times(
        Math.log10(player.replicanti.chance + 1) / Math.log10(2)
      );
  }

  data.est = Decimal.div(
    data.freq
      ? data.freq.times((Math.log10(2) / Math.log10(Math.E)) * 1e3)
      : Decimal.add(data.chance, 1).log(Math.E) * 1e3,
    data.interval
  );
  ...
  if (tmp.ngp3) {
    ...
    if (tmp.nrm !== 1 && tmp.qu.bigRip.active) {
      if (!player.dilation.active && tmp.qu.bigRip.upgrades.includes(14))
        tmp.nrm = tmp.nrm.pow(tmp.bru[14]);
      if (tmp.nrm.log10() > 1e9)
        tmp.nrm = Decimal.pow(10, 1e9 * Math.pow(tmp.nrm.log10() / 1e9, 2 / 3));
    }
    ...
    if (
      player.masterystudies.includes("d12") &&
      tmp.nf !== undefined &&
      tmp.nf.rewardsUsed !== undefined
    ) {
    ...
    if (player.masterystudies.includes("d10"))
      tmp.edgm = getEmperorDimensionGlobalMultiplier(); //Update global multiplier of all Emperor Dimensions
    tmp.be = tmp.qu.bigRip.active && tmp.qu.breakEternity.break;
    tmp.rg4 =
      tmp.qu.upgrades.includes("rg4") &&
      (tmp.qu.rg4 || !tmp.ngp3l || inQC(1) || QCIntensity(1));
...
function showTab(tabName, init) {
  if (tabName == "quantumtab" && !player.masterystudies) {
    alert(
      "Quantum was not fully implemented in NG++, and thus it is impossible to go further in this mod."
    );
    return;
  }
...
function getEternitied() {
  ...
  if (banked !== undefined && (inQC(0) || hasNU(10)))
    total = nA(total, player.eternitiesBank);
  return total;
}

function doGalaxyResetStuff(bulk) {
  player.money = player.achievements.includes("r111")
    ? player.money
    : new Decimal(10);
...
function galaxyReset(bulk) {
  ...
  if (player.spreadingCancer >= 10 && !player.achievements.includes("r41"))
  ...
  if (player.infinitied < 1 && player.eternities == 0 && !quantumed) {
    ...
    if (
      player.galacticSacrifice &&
      (player.galaxies > 0 ||
        (player.galacticSacrifice ? player.galacticSacrifice.times > 0 : false))
    ) {
  ...
  if (player.galaxies >= 50 && !player.achievements.includes("r83"))
    giveAchievement("YOU CAN GET 50 GALAXIES!??");
  if (player.galaxies >= 2 && !player.achievements.includes("r27"))
    giveAchievement("Double Galaxy");
  if (player.galaxies >= 1 && !player.achievements.includes("r26"))
  ...
  if (tmp.ngp3 && bulk) {
    ...
    if (tmp.qu.bigRip.active)
      tmp.qu.bigRip.bestGals = Math.max(
        tmp.qu.bigRip.bestGals,
        player.galaxies
      );
    if (ghostified && player.ghostify.neutrinos.boosts)
...
document.getElementById("secondSoftReset").onclick = function () {
  var bool =
    ...
    !((player.currentEternityChall == "eterc6" || inQC(6)) && !tmp.be) &&
    !tmp.ri &&
    !cantReset();
  if (
    ...
    if (
      (getEternitied() >= 7 || player.autobuyers[10].bulkBought) &&
      !shiftDown &&
...
function getGalaxyRequirement(offset = 0, display) {
  ...
  if (player.galacticSacrifice != undefined)
    amount -=
      player.galacticSacrifice.upgrades.includes(22) && player.galaxies > 0
        ? 80
        : 60;
  else if (
    inNC(6, 1) &&
    player.aarexModifications.ngexV != undefined &&
    tmp.grd.galaxies < 2
  )
    amount -= tmp.grd.galaxies == 1 ? 40 : 50;
  ...
  if (!player.boughtDims) {
    ...
    if (tmp.grd.galaxies >= 302500 / ghostlySpeed) {
      ...
      tmp.grd.speed = Math.pow(
        2,
        ((tmp.grd.galaxies + 1 - 302500 / ghostlySpeed) * ghostlySpeed) / div
      );
      scaling = 5;
    }
    ...
    if (tmp.grd.galaxies >= distantStart) {
      ...
      if (GUBought("gb6"))
        speed /= 1 + Math.pow(player.infinityPower.max(1).log10(), 0.25) / 2810;
      if (GUBought("br6")) speed /= 1 + player.meta.resets / 340;
      if (ghostified && player.ghostify.neutrinos.boosts > 5)
        speed /= tmp.nb[6];
      ...
      if (
        tmp.grd.galaxies >= distantStart * 2.5 &&
        player.galacticSacrifice != undefined
      ) {
        // 5 times worse scaling
        amount +=
          4 * speed * getDistantAdd(tmp.grd.galaxies - distantStart * 2.5 + 1);
    ...
    if (tmp.grd.galaxies >= remoteStart && !tmp.be && !hasNU(6)) {
      ...
      if (GUBought("gb7"))
        speed2 /=
          1 + Math.log10(1 + player.infinityPoints.max(1).log10()) / 100;
      if (GUBought("br7"))
        speed2 /= 1 + Math.log10(1 + player.eternityPoints.max(1).log10()) / 80;
      amount *= Math.pow(
        1 +
          (GUBought("rg1") ? 1 : 2) /
            (player.aarexModifications.ngmX > 3 ? 10 : 1e3),
        (tmp.grd.galaxies - remoteStart + 1) * speed2
      );
...
function getGalaxyReqMultiplier() {
  ...
  if (
    player.infinityUpgrades.includes("postinfi52") &&
    player.tickspeedBoosts == undefined
  )
    ret -= 3;
  ...
  if (tmp.grd.galaxies >= tmp.grd.darkStart) {
    ...
    if (GUBought("br5"))
      push *=
        1 +
        Math.min(
          Math.sqrt(player.dilation.tachyonParticles.max(1).log10()) * 0.013,
          0.14
        );
...
function getRemoteScalingStart(galaxies) {
  ...
  if (tmp.ngp3) {
    for (var t = 251; t < 254; t++)
      if (player.masterystudies.includes("t" + t)) n += getMTSMult(t);
...
function updateWorstChallengeBonus() {
  ...
  worstChallengeBonus = Decimal.max(
    Math.pow(
      3000 / Math.max(33e-6, worstChallengeTime * 0.1),
      player.galacticSacrifice ? 2 : 1
    ),
    1
  );
}
...
function getDilPower() {
  ...
  if (tmp.ngp3) {
    if (player.achievements.includes("ng3p11") && !tmp.ngp3l)
      ret = ret.times(Math.max(getTotalRG() / 125, 1));
...
function getDilExp(disable) {
  let ret = 1.5;
  if (player.aarexModifications.newGameExpVersion) ret += 0.001;
  if (player.meta !== undefined && !player.aarexModifications.nguspV)
    ret += getDilUpgPower(4) / 4;
  if (tmp.ngp3) {
    if (
      (!tmp.qu.bigRip.active || tmp.qu.bigRip.upgrades.includes(11)) &&
      player.masterystudies.includes("d13") &&
      disable != "TU3"
    )
...
function getDilGain() {
  ...
  var log =
    Math.log10(player.money.log10() / 400) * getDilExp() +
    getDilPower().log10();
...
function getNGUDTGain() {
  ...
  if (player.eternityUpgrades.includes(8))
    gain = gain.times(
      1 + Math.log10(Math.max(1, player.infinityPoints.log(10))) / 20
    );
  if (player.eternityUpgrades.includes(9))
    gain = gain.times(
      1 + Math.log10(Math.max(1, player.eternityPoints.log(10))) / 10
    );
  return gain;
}
...
function getEternityBoostToDT() {
  ...
  if (eterExp > 0)
    gain = gain.times(Decimal.max(getEternitied(), 1).pow(eterExp));
  if (
    player.dilation.upgrades.includes("ngpp2") &&
    player.aarexModifications.newGameExpVersion
  ) {
    let e = new Decimal(getEternitied());
    gain = gain
      .times(e.max(10).log10())
      .times(Math.pow(e.max(1e7).log10() - 6, 3));
...
function getDilTimeGainPerSecond() {
  ...
  if (
    ...
    if (player.achievements.includes("ng3p11") && !tmp.ngp3l)
      gain = gain.times(Math.max(player.galaxies / 600 + 0.5, 1));
    if (player.achievements.includes("ng3p41") && !tmp.ngp3l)
      gain = gain.times(
        Decimal.pow(4, Math.sqrt(player.quantum.nanofield.rewards))
      );
    ...
    if (GUBought("br2"))
      gain = gain.times(
        Decimal.pow(2.2, Math.pow(tmp.sacPow.max(1).log10() / 1e6, 0.25))
      );
    if (!tmp.ngp3l)
      gain = gain.times(
  ...
  if (tmp.newNGP3E && player.achievements.includes("r138") && gain.lt(1e100))
    gain = gain.times(3);
  if (
    !tmp.ngp3l &&
    (tmp.ngp3 || tmp.newNGP3E) &&
...
function getEternitiesAndDTBoostExp() {
  ...
  if (player.dilation.upgrades.includes("ngpp2"))
    exp += player.aarexModifications.ngudpV ? 0.2 : 0.1;
  if (player.dilation.upgrades.includes("ngud2")) exp += 0.1;
  if (player.dilation.upgrades.includes("ngmm3")) exp += 0.1;
  return exp;
}
...
function dimShiftDisplay() {
  ...
  var isShift =
    player.resets <
    (inNC(4) || player.currentChallenge == "postc1" || player.pSac !== undefined
      ? 2
      : 4);
  document.getElementById("resetLabel").textContent =
    ...
    (isShift
      ? "Shift"
      : player.resets < getSupersonicStart()
      ? "Boost"
      : "Supersonic") +
    " (" +
    getFullExpansion(Math.ceil(player.resets)) +
    "): requires " +
    getFullExpansion(Math.ceil(shiftRequirement.amount)) +
    " " +
    DISPLAY_NAMES[shiftRequirement.tier] +
    " Dimensions";
  document.getElementById("softReset").textContent =
    "Reset the game for a " + (isShift ? "new dimension" : "boost");
}
...
function tickspeedBoostDisplay() {
  if (isTickspeedBoostPossible()) {
    ...
    document.getElementById("tickResetLabel").textContent =
      "Tickspeed Boost (" +
      getFullExpansion(player.tickspeedBoosts) +
      "): requires " +
      getFullExpansion(tickReq.amount) +
      " " +
      DISPLAY_NAMES[tickReq.tier] +
      " Dimensions";
    document.getElementById("tickResetBtn").className =
      getAmount(tickReq.tier) < tickReq.amount ? "unavailablebtn" : "storebtn";
...
function galaxyReqDisplay() {
  ...
  var totalTypes = tmp.aeg
    ...
    ? 2
    : 1;
  document.getElementById("secondResetLabel").innerHTML =
    getGalaxyScaleName(nextGal.scaling) +
    (nextGal.scaling <= 3 ? "Antimatter " : "") +
    " Galaxies (" +
    getFullExpansion(player.galaxies) +
    (totalTypes > 1 ? " + " + getFullExpansion(totalReplGalaxies) : "") +
    (totalTypes > 2
      ? " + " + getFullExpansion(Math.round(player.dilation.freeGalaxies))
      : "") +
    (totalTypes > 3 ? " + " + getFullExpansion(tmp.aeg) : "") +
    "): requires " +
    getFullExpansion(nextGal.amount) +
    " " +
    DISPLAY_NAMES[inNC(4) || player.pSac != undefined ? 6 : 8] +
    " Dimensions";
}
...
function intergalacticDisplay() {
  ...
  if (player.achievements.includes("ng3p37") && shiftRequirement.tier > 7) {
    ...
    document.getElementById("intergalacticLabel").innerHTML =
      getGalaxyScaleName(tmp.igs) +
      "Intergalactic Boost " +
      (player.dilation.active || player.galacticSacrifice != undefined
        ? " (estimated)"
        : "") +
      " (" +
      getFullExpansion(player.galaxies) +
      (Math.floor(tmp.igg - player.galaxies) > 0
        ? " + " + getFullExpansion(Math.floor(tmp.igg - player.galaxies))
        : "") +
      "): " +
      shorten(
        dilates(tmp.ig).pow(
          player.dilation.active ? getNanofieldRewardEffect(5, "dil_exp") : 1
        )
      ) +
...
function dimensionTabDisplay() {
  ...
  for (let tier = 8; tier > 0; tier--) {
    ...
    if (shown) {
      ...
      document.getElementById("D" + tier).childNodes[0].nodeValue =
        DISPLAY_NAMES[tier] +
        " Dimension x" +
        formatValue(
          player.options.notation,
      ...
      document.getElementById("A" + tier).textContent = getDimensionDescription(
        tier
      );
    }
  }
  setAndMaybeShow(
    ...
    "'Multiplier per 10 dimensions: '+shorten(getDimensionPowerMultiplier(\"non-random\"))+'x'"
  );
  dimShiftDisplay();
  ...
  if (canBuyDimension(3) || player.currentEternityChall == "eterc9") {
    ...
    if (isNaN(tickmultNum)) ticklabel = "Break the tick interval by Infinite";
      ...
      ticklabel =
        ...
        ((1 - tickmultNum) * 100).toFixed(e) +
        "%";
    let ic3mult = getPostC3Mult();
    if (
      player.galacticSacrifice ||
      player.currentChallenge == "postc3" ||
      isIC3Trapped()
    )
      document.getElementById("tickLabel").innerHTML =
        ((isIC3Trapped() || player.currentChallenge == "postc3") &&
        player.currentChallenge != "postcngmm_3" &&
        !player.challenges.includes("postcngmm_3") &&
        !tmp.be
          ? "M"
          : ticklabel + "<br>and m") +
        "ultiply all dimensions by " +
        (ic3mult > 999.95
          ? shorten(ic3mult)
...
function paradoxDimDisplay() {
  document.getElementById("pPow").textContent = shortenMoney(
    player.pSac.dims.power
  );
  document.getElementById("pPowProduction").textContent =
    "You are getting " +
    shortenDimensions(getPDProduction(1).div(getEC12Mult())) +
    " Paradox Power per second.";
  document.getElementById("pPowEffect").textContent = getFullExpansion(
    Math.floor(getExtraTime() * getEC12Mult())
  );
  var shown;
  for (let t = 8; t > 0; t--) {
    ...
    if (shown) {
      document.getElementById("pD" + t).textContent =
        DISPLAY_NAMES[t] + " Paradox Dimension x" + shortenMoney(getPDPower(t));
      document.getElementById("pB" + t).textContent =
        "Cost: " + shortenDimensions(player.pSac.dims[t].cost) + " Px";
      document.getElementById("pB" + t).className =
        (player.pSac.px.gte(player.pSac.dims[t].cost) ? "stor" : "unavailabl") +
        "ebtn";
...
function mainStatsDisplay() {
  ...
  document.getElementById("totalresets").textContent =
    "You have done " +
    getFullExpansion(player.resets) +
    " dimension boosts/shifts.";
  setAndMaybeShow(
    "lostResets",
    player.pSac && player.pSac.lostResets,
    '"You have lost a total of "+getFullExpansion(player.pSac.lostResets)+" dimension boosts/shifts only after matter exceeded your antimatter."'
  );
  document.getElementById("tdboosts").textContent =
    player.aarexModifications.ngmX > 3
      ? "You have done " +
        getFullExpansion(player.tdBoosts) +
        " time dimension boosts/shifts."
      : "";
  ...
  document.getElementById("galaxies").textContent =
    "You have " + getFullExpansion(player.galaxies) + " Antimatter Galaxies.";
  var showCancer = player.spreadingCancer > 0 && player.galacticSacrifice;
  document.getElementById("spreadingCancer").style.display = showCancer
    ? ""
    : "none";
  if (showCancer)
    document.getElementById("spreadingCancer").textContent =
      "You have made " +
      getFullExpansion(player.spreadingCancer) +
      " total galaxies while using Cancer notation.";
...
function paradoxSacDisplay() {
  if (player.pSac !== undefined && player.pSac.times) {
    ...
    document.getElementById("pSacrificedNormal").textContent =
      "You have Paradox Sacrificed " +
      getFullExpansion(player.pSac.normalTimes) +
      " times.";
    document.getElementById("pSacrificedForced").textContent =
      "You have been forced to do a Paradox Sacrifice " +
      getFullExpansion(player.pSac.forcedTimes) +
      " times.";
    document.getElementById("pSacrificed").textContent =
      "You have Paradox Sacrificed a total of " +
      getFullExpansion(player.pSac.times) +
      " times.";
    document.getElementById("thisPSac").textContent =
      "You have spent " +
      timeDisplay(player.pSac.time) +
      " in this Paradox Sacrifice.";
...
function galaxySacDisplay() {
  if (player.galacticSacrifice ? player.galacticSacrifice.times < 1 : true)
    ...
    document.getElementById("sacrificed").textContent =
      "You have galactic sacrificed " +
      getFullExpansion(player.galacticSacrifice.times) +
      " times.";
    document.getElementById("thisSacrifice").textContent =
      "You have spent " +
      timeDisplay(player.galacticSacrifice.time) +
      " in this galactic sacrifice.";
  }
}
...
function bestInfinityDisplay() {
  ...
  if (player.bestInfinityTime == 9999999999) {
    ...
    document.getElementById("infinitied").textContent =
      "You have infinitied " +
      getFullExpansion(player.infinitied) +
      " time" +
      (player.infinitied == 1 ? "" : "s") +
      (player.eternities !== 0 || player.eternitiesBank > 0
        ? " this eternity."
        : ".");
  }
...
function bestEternityDisplay() {
  ...
  if (player.eternities == 0) {
    ...
    document.getElementById("eternitied").textContent =
      "You have Eternitied " +
      getFullExpansion(player.eternities) +
      " time" +
      (player.eternities == 1 ? "" : "s") +
      (quantumed ? " this quantum." : ".");
    document.getElementById("besteternity").textContent =
      "You have spent " +
...
function bestQuantumDisplay() {
  if (!quantumed)
    ...
    document.getElementById("quantumed").textContent =
      "You have gone quantum " + getFullExpansion(tmp.qu.times) + " times.";
    document.getElementById("thisQuantum").textContent =
      "You have spent " + timeDisplay(tmp.qu.time) + " in this quantum.";
    document.getElementById("bestQuantum").textContent =
      "Your fastest quantum is in " + timeDisplay(tmp.qu.best) + ".";
  }
}
...
function bestGhostifyDisplay() {
  if (!ghostified)
    ...
    document.getElementById("ghostified").textContent =
      "You have became a ghost and passed big ripped universes " +
      getFullExpansion(player.ghostify.times) +
      " times.";
    document.getElementById("thisGhostify").textContent =
      "You have spent " +
      timeDisplay(player.ghostify.time) +
      " in this Ghostify.";
    document.getElementById("bestGhostify").textContent =
      "Your fastest Ghostify is in " + timeDisplay(player.ghostify.best) + ".";
  }
}
...
function ng3p51Display() {
  if (!player.achievements.includes("ng3p51"))
    ...
    setAndMaybeShow(
      "bigRipped",
      tmp.qu.bigRip.times,
      '"You have big ripped the universe "+getFullExpansion(tmp.qu.bigRip.times)+" times."'
    );
    setAndMaybeShow(
      "bestmoneythisrip",
      tmp.qu.bigRip.active,
      "'Your best antimatter in this Big Rip is ' + shortenMoney(tmp.qu.bigRip.bestThisRun) + '.'"
    );
    document.getElementById("totalmoneybigrip").textContent =
      "You have made a total of " +
      shortenMoney(tmp.qu.bigRip.totalAntimatter) +
      " antimatter in all big rips.";
    document.getElementById("bestgalsbigrip").textContent =
      "Your best amount of normal galaxies for all Big Rips is " +
      getFullExpansion(tmp.qu.bigRip.bestGals) +
      ".";
  }
}
...
function dilationStatsDisplay() {
  if (player.dilation.times)
    document.getElementById("dilated").textContent =
      "You have succesfully dilated " +
      getFullExpansion(player.dilation.times) +
      " times.";
  ...
  if (player.exdilation == undefined ? false : player.exdilation.times > 1)
    document.getElementById("exdilated").textContent =
      "You have reversed dilation " +
      getFullExpansion(player.exdilation.times) +
      " times.";
...
function infinityRespecedInfinityDisplay() {
  if (setUnlocks.length > player.setsUnlocked)
    document.getElementById("nextset").textContent =
      ...
      formatValue(
        player.options.notation,
        setUnlocks[player.setsUnlocked],
        2,
        0,
        true
      ) +
      ".";
  document.getElementById("infi1pow").textContent = getFullExpansion(
    player.infinityUpgradesRespecced[1] * 10
  );
  document.getElementById("infi1cost").textContent = shortenCosts(
    Decimal.pow(10, player.infinityUpgradesRespecced[1])
  );
  document.getElementById("infi1").className = player.infinityPoints.lt(
    Decimal.pow(10, player.infinityUpgradesRespecced[1])
  )
    ? "infinistorebtnlocked"
    : "infinimultbtn";
  document.getElementById("infi3pow").textContent = formatValue(
    player.options.notation,
    getLimit(),
    2,
    0,
  ...
  document.getElementById("infi3cost").textContent = shortenCosts(
    Decimal.pow(10, player.infinityUpgradesRespecced[3])
  );
  document.getElementById("infi3").className = player.infinityPoints.lt(
    Decimal.pow(10, player.infinityUpgradesRespecced[3])
  )
    ? "infinistorebtnlocked"
    : "infinimultbtn";
...
function infinityUpgradesDisplay() {
  ...
  if (player.infinityUpgrades.includes("18Mult"))
    ...
    player.infinityPoints.gte(1) &&
    player.infinityUpgrades.includes("timeMult")
  )
  ...
  if (player.infinityUpgrades.includes("27Mult"))
    ...
    player.infinityPoints.gte(1) &&
    player.infinityUpgrades.includes("dimMult")
  )
  ...
  if (player.infinityUpgrades.includes("36Mult"))
    ...
    player.infinityPoints.gte(1) &&
    player.infinityUpgrades.includes("18Mult")
  )
  ...
  if (player.infinityUpgrades.includes("45Mult"))
    ...
    player.infinityPoints.gte(1) &&
    player.infinityUpgrades.includes("27Mult")
  )
  ...
  if (player.infinityUpgrades.includes("resetBoost"))
    ...
    player.infinityPoints.gte(1) &&
    player.infinityUpgrades.includes("36Mult")
  )
  ...
  document.getElementById("infi11").innerHTML =
    "Normal Dimensions gain a multiplier based on time played in this save file<br>Currently: " +
    infUpg11Pow().toFixed(2) +
    "x<br>Cost: 1 IP";
  document.getElementById("infi12").innerHTML =
    "First and Eighth Dimensions gain a multiplier based on your Infinities<br>Currently: " +
    formatValue(player.options.notation, dimMults(), 1, 1) +
    "x<br>Cost: 1 IP";
  document.getElementById("infi13").innerHTML =
    "Third and Sixth Dimensions gain a multiplier based on your Infinities<br>Currently: " +
    formatValue(player.options.notation, dimMults(), 1, 1) +
    "x<br>Cost: 1 IP";
  document.getElementById("infi22").innerHTML =
    "Second and Seventh Dimensions gain a multiplier based on your Infinities<br>Currently: " +
    formatValue(player.options.notation, dimMults(), 1, 1) +
    "x<br>Cost: 1 IP";
  document.getElementById("infi23").innerHTML =
    "Fourth and Fifth Dimensions gain a multiplier based on your Infinities<br>Currently: " +
    formatValue(player.options.notation, dimMults(), 1, 1) +
    "x<br>Cost: 1 IP";
  document.getElementById("infi31").innerHTML =
    "Normal Dimensions gain a multiplier based on time spent in this Infinity<br>Currently: " +
    shorten(infUpg13Pow()) +
    "x<br>Cost: 3 IP";
  var infi32middle =
    player.infinityPoints.l < 1e10
      ? "<br>Currently: " +
        formatValue(player.options.notation, getUnspentBonus(), 2, 2) +
        "x"
      : "";
  document.getElementById("infi32").innerHTML =
    "Multiplier for unspent Infinity Points on 1st Dimension" +
    infi32middle +
    "<br>Cost: 5 IP";
}
...
function preBreakUpgradeDisplay() {
  ...
  document.getElementById("infiMult").innerHTML =
    "You get 2x more IP. " +
    Math.round(getIPMultPower() * 100) / 100 +
    infiMultEnding;
  document.getElementById("nextset").textContent = "";
  if (player.infinityUpgradesRespecced != undefined) {
    ...
    if (player.galacticSacrifice) {
      ...
      document.getElementById("infi21").innerHTML =
        "Increase the multiplier for buying 10 Dimensions based on your Infinities<br>" +
        base +
        "x -> " +
        (infUpg12Pow() * base).toPrecision(4) +
        "x<br>Cost: 1 IP";
      document.getElementById("infi33").innerHTML =
        "Dimension boosts are stronger based on IP<br>Currently: " +
        (1.2 + 0.05 * player.infinityPoints.max(1).log(10)).toFixed(2) +
        "x<br>Cost: 7 IP";
    }
    var infi34Middle =
      player.infinityPoints.l < 1e10
        ? "<br>Currently: " +
          shortenDimensions(getIPMult()) +
          " every " +
          timeDisplay(player.bestInfinityTime * 10)
        : "";
    document.getElementById("infi34").innerHTML =
      "You generate IP based on your fastest Infinity time " +
      infi34Middle +
      "<br>Cost: 10 IP";
  }
  ...
  if (player.setsUnlocked > 0) {
    ...
    for (u = 4; u < 7; u++) {
      document.getElementById("infi" + u + "pow").textContent =
        u == 5 ? getInfUpgPow(5).toFixed(2) : getFullExpansion(getInfUpgPow(u));
      document.getElementById("infi" + u + "cost").textContent = shortenCosts(
        Decimal.pow(10, player.infinityUpgradesRespecced[u] + powAdds[u])
      );
      document.getElementById("infi" + u).className = player.infinityPoints.lt(
        Decimal.pow(10, player.infinityUpgradesRespecced[u] + powAdds[u])
      )
        ? "infinistorebtnlocked"
        : "infinimultbtn";
...
function infoScaleDisplay() {
  if (player.aarexModifications.hideRepresentation)
    ...
    player.money.gt(Decimal.pow(10, (3 * 86400 * 365.2425 * 79.3) / 10))
  ) {
    var years = player.money.log10() / 3 / 86400 / 365.2425;
    var todayYear = new Date().getFullYear() || 2020;
    if (years >= 1e12)
      var message =
        "<br>If you end the non-stop writing of your full antimatter amount with 3 digits per second, you would start it " +
        (years / 1e12).toFixed(2) +
        " TYA.";
    else if (years >= 1e9)
      var message =
        "<br>If you end the non-stop writing of your full antimatter amount with 3 digits per second, you would start it " +
      ...
      // bound to the  < } else { > line stuck btw two var message lines
      ...
      if (index > 0) {
        //bc is less than or equal to 5332e3
        since = events[index - 1];
        sinceYears = dates[index] - bc;
      }
      var message =
        "<br>If you end the non-stop writing of your full antimatter amount with 3 digits per second, you would start it in " +
        getFullExpansion(Math.floor(bc)) +
        " BC." +
        (since
          ? "<br>(around " +
            getFullExpansion(Math.ceil(sinceYears)) +
            " years since the " +
            since +
            ")"
          : "");
      ...
      var message =
        "<br>If you wrote 3 digits of your full antimatter amount every second since you were born as American,<br> you would ";
      if (years > 79.3)
        message +=
          "be a ghost for " +
        ...
        message +=
          ...
          "% of your projected average lifespan.";
    }
...
function breakInfinityUpgradeDisplay() {
  ...
  document.getElementById("postinfi11").innerHTML =
    "Power up Normal Dimensions based on total antimatter produced<br>Currently: " +
    shorten(
      Math.pow(player.totalmoney.e + 1, player.galacticSacrifice ? 2 : 0.5)
    ) +
    "x<br>Cost: " +
    shortenCosts(1e4) +
    " IP";
  document.getElementById("postinfi21").innerHTML =
    "Power up Normal Dimensions based on your current antimatter<br>Currently: " +
    shorten(Math.pow(player.money.e + 1, player.galacticSacrifice ? 2 : 0.5)) +
    "x<br>Cost: " +
    shortenCosts(5e4) +
    " IP";
  if (player.tickSpeedMultDecrease > 2)
  ...
  document.getElementById("postinfi22").innerHTML =
    "Power up Normal Dimensions based on achievements completed <br>Currently: " +
    shorten(achievementMult) +
    "x<br>Cost: " +
    shortenCosts(1e6) +
    " IP";
  document.getElementById("postinfi12").innerHTML =
    "Power up Normal Dimensions based on your infinities<br>Currently: " +
    shorten(getInfinitiedMult()) +
    "x<br>Cost: " +
    shortenCosts(1e5) +
    " IP";
  document.getElementById("postinfi41").innerHTML =
    "Make galaxies " +
    Math.round(getPostGalaxyEff() * 100 - 100) +
    "% stronger <br>Cost: " +
    shortenCosts(5e11) +
    " IP";
  document.getElementById("postinfi32").innerHTML =
    "Power up Normal Dimensions based on your slowest normal challenge run time<br>Currently: " +
    shorten(worstChallengeBonus) +
    "x<br>Cost: " +
    shortenCosts(1e7) +
    " IP";
  document.getElementById("postinfi13").innerHTML =
    "You passively generate Infnities based on your fastest infinity.<br>1 Infinity every " +
    timeDisplay(player.bestInfinityTime * 5) +
    " <br>Cost: " +
    shortenCosts(2e7) +
    " IP";
  document.getElementById("postinfi23").innerHTML =
    "Unlock the option to bulk buy Dimension" +
    (player.tickspeedBoosts == undefined ? "" : " and Tickspeed") +
    " Boosts <br>Cost: " +
    shortenCosts(
      player.tickspeedBoosts != undefined
        ? 2e4
  ...
  document.getElementById("offlineProd").innerHTML =
    ...
    "% of your best IP/min from the last 10 infinities, works offline<br>Currently: " +
...
function breakNGm2UpgradeColumnDisplay() {
  if (player.infinityUpgrades.includes("galPointMult"))
    document.getElementById("postinfi01").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 1e3 : 1e4)
  )
    document.getElementById("postinfi01").className = "infinistorebtn1";
  else document.getElementById("postinfi01").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("dimboostCost"))
    document.getElementById("postinfi02").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 2e4 : 1e5)
  )
    document.getElementById("postinfi02").className = "infinistorebtn1";
  else document.getElementById("postinfi02").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("galCost"))
    document.getElementById("postinfi03").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte(5e5))
    document.getElementById("postinfi03").className = "infinistorebtn1";
  else document.getElementById("postinfi03").className = "infinistorebtnlocked";
  if (player.extraDimPowerIncrease >= 40)
    document.getElementById("postinfi04").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte(player.dimPowerIncreaseCost))
    document.getElementById("postinfi04").className = "infinimultbtn";
  else document.getElementById("postinfi04").className = "infinistorebtnlocked";
  document.getElementById("postinfi01").innerHTML =
    "Gain a multiplier to galaxy points based on your infinities<br>Currently: " +
    shorten(getPost01Mult()) +
    "x<br>Cost: " +
    shortenCosts(player.tickspeedBoosts == undefined ? 1e3 : 1e4) +
    " IP";
  document.getElementById("postinfi02").innerHTML =
    "Dimension boost cost increases by 1 less<br>Currently: " +
    getDimboostCostIncrease() +
    (player.infinityUpgrades.includes("dimboostCost")
      ? ""
      : " -> " + (getDimboostCostIncrease() - 1)) +
    "<br>Cost: " +
    shortenCosts(player.tickspeedBoosts == undefined ? 2e4 : 1e5) +
    " IP";
  document.getElementById("postinfi03").innerHTML =
    "Galaxy cost increases by 5 less<br>Currently: " +
    Math.round(getGalaxyReqMultiplier() * 10) / 10 +
    (player.infinityUpgrades.includes("galCost")
      ? ""
      : " -> " +
        Math.round(getGalaxyReqMultiplier() * 10 - 50) / 10 +
        "<br>Cost: " +
        shortenCosts(5e5) +
        " IP");
  document.getElementById("postinfi04").innerHTML =
    "Further increase all dimension multipliers<br>x^" +
    galMults.u31().toFixed(2) +
    (player.extraDimPowerIncrease < 40
      ? " -> x^" +
        (galMults.u31() + 0.02).toFixed(2) +
        "<br>Cost: " +
        shorten(player.dimPowerIncreaseCost) +
        " IP"
      : "");
}
...
function breakNGm2UpgradeRow5Display() {
  ...
  if (player.infinityUpgrades.includes("postinfi50"))
    document.getElementById("postinfi50").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 1e25 : 1e18)
  )
    document.getElementById("postinfi50").className = "infinistorebtn1";
  else document.getElementById("postinfi50").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi51"))
    document.getElementById("postinfi51").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 1e29 : 1e20)
  )
    document.getElementById("postinfi51").className = "infinistorebtn1";
  else document.getElementById("postinfi51").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi52"))
    document.getElementById("postinfi52").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 1e33 : 1e25)
  )
    document.getElementById("postinfi52").className = "infinistorebtn1";
  else document.getElementById("postinfi52").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi53"))
    document.getElementById("postinfi53").className = "infinistorebtnbought";
  else if (
    player.infinityPoints.gte(player.tickspeedBoosts == undefined ? 1e37 : 1e29)
  )
    document.getElementById("postinfi53").className = "infinistorebtn1";
  else document.getElementById("postinfi53").className = "infinistorebtnlocked";
  document.getElementById("postinfi50").innerHTML =
    "Dimension boost cost increases by 0.5 less.<br>Currently: " +
    getDimboostCostIncrease() +
    (player.infinityUpgrades.includes("postinfi50")
      ? ""
      : " -> " + (getDimboostCostIncrease() - 0.5)) +
    "<br>Cost: " +
    shortenCosts(player.tickspeedBoosts == undefined ? 1e25 : 1e18) +
    " IP";
  document.getElementById("postinfi51").innerHTML =
    "Galaxies are " +
    (player.tickspeedBoosts != undefined ? 15 : 20) +
    "% more stronger.<br>Cost: " +
    shortenCosts(player.tickspeedBoosts == undefined ? 1e29 : 1e20) +
    " IP";
  let inf52text = "";
  if (player.tickspeedBoosts == undefined) {
    inf52text =
      "Galaxy cost increases by 3 less.<br>Currently: " +
      Math.round(getGalaxyReqMultiplier() * 10) / 10 +
      (player.infinityUpgrades.includes("postinfi52")
        ? ""
        : " -> " + Math.round(getGalaxyReqMultiplier() * 10 - 30) / 10) +
      "<br>Cost: " +
      shortenCosts(1e33) +
      " IP";
  } else
    inf52text =
      "Decrease tickspeed boost cost multiplier to 3.<br>Cost: " +
      shortenCosts(1e25) +
      " IP";
  document.getElementById("postinfi52").innerHTML = inf52text;
  document.getElementById("postinfi53").innerHTML =
    "Divide all Infinity Dimension cost multipliers by 50.<br>Cost: " +
    shortenCosts(player.tickspeedBoosts == undefined ? 1e37 : 1e29) +
    " IP";
}

function breakNGm2UpgradeRow6Display() {
  document.getElementById("postinfir6").style.display = "";
  if (player.infinityUpgrades.includes("postinfi60"))
    document.getElementById("postinfi60").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte(1e50))
    document.getElementById("postinfi60").className = "infinistorebtn1";
  else document.getElementById("postinfi60").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi61"))
    document.getElementById("postinfi61").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte("1e450"))
    document.getElementById("postinfi61").className = "infinistorebtn1";
  else document.getElementById("postinfi61").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi62"))
    document.getElementById("postinfi62").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte("1e700"))
    document.getElementById("postinfi62").className = "infinistorebtn1";
  else document.getElementById("postinfi62").className = "infinistorebtnlocked";
  if (player.infinityUpgrades.includes("postinfi63"))
    document.getElementById("postinfi63").className = "infinistorebtnbought";
  else if (player.infinityPoints.gte("1e2000"))
    document.getElementById("postinfi63").className = "infinistorebtn1";
  else document.getElementById("postinfi63").className = "infinistorebtnlocked";
  document.getElementById("postinfi60").innerHTML =
    "You gain more " +
    (player.tickspeedBoosts != undefined
      ? "Galaxy Points"
      : tmp.ngp3l
      ? "Infinity Points"
      : "antimatter") +
    " based on your galaxies." +
    (player.tickspeedBoosts != undefined
      ? ""
      : "<br>Currently: " + shorten(getNewB60Mult()) + "x") +
    "<br>Cost: " +
    shortenCosts(1e50) +
    " IP";
  document.getElementById("postinfi61").innerHTML =
    "g11 formula is better.<br>Cost: " +
    shortenCosts(new Decimal("1e450")) +
    " IP";
  document.getElementById("postinfi62").innerHTML =
    "Dimension Boosts make g13 stronger.<br>Cost: " +
    shortenCosts(new Decimal("1e700")) +
    " IP";
  document.getElementById("postinfi63").innerHTML =
    "Unlock 2 new rows of Galaxy Upgrades.<br>Cost: " +
...
function INFINITYUPGRADESDisplay() {
  if (document.getElementById("preinf").style.display == "block") {
    ...
    document.getElementById("postinf").style.display == "block" &&
    ...
    if (
      player.galacticSacrifice &&
      (player.infinityDimension3.amount.gt(0) ||
        player.eternities >
          (player.aarexModifications.newGameMinusVersion ? -20 : 0) ||
        quantumed)
    ) {
    ...
    if (
      player.galacticSacrifice &&
      (player.infinityDimension4.amount.gt(0) ||
        player.eternities >
          (player.aarexModifications.newGameMinusVersion ? -20 : 0) ||
        quantumed)
    ) {
    ...
    document.getElementById("darkMatter").textContent = shortenMoney(
      player.singularity.darkMatter
    );
    document.getElementById("darkMatterMult").textContent = shortenMoney(
      getDarkMatterMult()
    );
  } else if (
    document.getElementById("dimtechs").style.display == "block" &&
    document.getElementById("dimtechsdiv").style.display == ""
  ) {
    document.getElementById("darkMatterDT").textContent = shortenMoney(
      player.singularity.darkMatter
    );
    document.getElementById("nextDiscounts").textContent = shortenMoney(
      getNextDiscounts()
    );
    document.getElementById("discounts").textContent =
      "You have gained a total of " +
      getFullExpansion(player.dimtechs.discounts) +
      " discount upgrades.";
  }
}
...
function eternityUpgradesDisplay() {
  ...
  document.getElementById("eter1").innerHTML =
    "Infinity Dimension multiplier based on unspent EP (x+1)<br>Currently: " +
  ...
  document.getElementById("eter2").innerHTML =
    ...
    eu2formula +
    ")<br>Currently: " +
    shortenMoney(getEU2Mult()) +
    "x<br>Cost: 10 EP";
  document.getElementById("eter3").innerHTML =
    "Infinity Dimension multiplier based on " +
    (player.boughtDims
      ? "time shards (x/" + shortenCosts(1e12) + "+1)"
      : "sum of Infinity Challenge times") +
    "<br>Currently: " +
    shortenMoney(getEU3Mult()) +
    "x<br>Cost: " +
    shortenCosts(5e4) +
    " EP";
  ...
  document.getElementById("eter5").innerHTML =
    "Time Dimensions gain a multiplier based on your unspent Time Theorems" +
    "<br>Cost: " +
    shortenCosts(1e40) +
    " EP";
  document.getElementById("eter6").innerHTML =
    "Time Dimensions gain a multiplier based on days played" +
    "<br>Cost: " +
    shortenCosts(1e50) +
    " EP";
  if (player.exdilation != undefined && player.dilation.studies.includes(1)) {
    document.getElementById("eter7").innerHTML =
      ...
      (1 + Math.log10(Math.max(1, player.money.log(10))) / 40).toFixed(3) +
      "x<br>Cost: " +
    ...
    document.getElementById("eter8").innerHTML =
      ...
      (1 + Math.log10(Math.max(1, player.infinityPoints.log(10))) / 20).toFixed(
        3
      ) +
      "x<br>Cost: " +
      shortenCosts(new Decimal("1e2000")) +
    ...
    document.getElementById("eter9").innerHTML =
      ...
      (1 + Math.log10(Math.max(1, player.eternityPoints.log(10))) / 10).toFixed(
        3
      ) +
      "x<br>Cost: " +
      shortenCosts(new Decimal("1e3000")) +
      " EP";
  }
}
...
function uponDilationDisplay() {
  ...
  if (player.infinityPoints.lt(Number.MAX_VALUE) || inQCModifier("ad")) {
    ...
    msg +=
      " for " +
      shortenMoney(gain.sub(player.dilation.totalTachyonParticles)) +
      " Tachyon particles";
...
function exdilationDisplay() {
  ...
  if (canReverseDilation()) {
    ...
    document.getElementById("reversedilation").innerHTML =
      "Reverse dilation." +
      (player.exdilation.times > 0 || quantumed
        ? "<br>Gain " + shortenDimensions(getExDilationGain()) + " ex-dilation"
        : "");
  } else {
    let req = getExdilationReq();
    document.getElementById("reversedilation").className =
      "eternityupbtnlocked";
    document.getElementById("reversedilation").textContent =
      "Get " +
      (player.eternityPoints.lt(req.ep)
        ? shortenCosts(new Decimal(req.ep)) + " EP and "
        : "") +
      shortenCosts(req.dt) +
      " dilated time to reverse dilation.";
  }
}
  ...
  if (player.dilation.active) uponDilationDisplay();
    ...
    document.getElementById("enabledilation").textContent =
      ...
      ((
        ...
        player.eternityBuyer.dilMode == "amount"
          ? !isNaN(player.eternityBuyer.statBeforeDilation)
          : false
      )
        ? " " +
        ...
        : "");
  if (
    player.exdilation == undefined || player.aarexModifications.ngudpV
      ? false
      : player.blackhole.unl
  ) {
    exdilationDisplay();
  } else document.getElementById("reversedilationdiv").style.display = "none";
  var fgm = getFreeGalaxyGainMult();
  document.getElementById("freeGalaxyMult").textContent =
    fgm == 1 ? "free galaxy" : Math.round(fgm * 10) / 10 + " free galaxies";
}

function breakEternityDisplay() {
  document.getElementById("eternalMatter").textContent = shortenDimensions(
    tmp.qu.breakEternity.eternalMatter
  );
  for (var u = 1; u < (player.ghostify.ghostlyPhotons.unl ? 11 : 8); u++) {
    document.getElementById("breakUpg" + u).className =
      tmp.qu.breakEternity.upgrades.includes(u) && u != 7
        ? "eternityupbtnbought"
        : tmp.qu.breakEternity.eternalMatter.gte(getBreakUpgCost(u))
        ? "eternityupbtn"
        : "eternityupbtnlocked";
    if (u == 8)
      document.getElementById("breakUpg8Mult").textContent = (
        getBreakUpgMult(8) * 100 -
        100
      ).toFixed(1);
    else if (u != 7)
      document.getElementById(
        "breakUpg" + u + "Mult"
      ).textContent = shortenMoney(getBreakUpgMult(u));
  }
  if (tmp.qu.bigRip.active) {
    document.getElementById("eterShortcutEM").textContent = shortenDimensions(
      tmp.qu.breakEternity.eternalMatter
    );
    document.getElementById("eterShortcutEP").textContent = shortenDimensions(
      player.eternityPoints
    );
    document.getElementById("eterShortcutTP").textContent = shortenMoney(
...
function ETERNITYSTOREDisplay() {
  ...
  if (document.getElementById("blackhole").style.display == "block") {
    ...
    if (
      ...
      document.getElementById("blackholeunlock").innerHTML =
        "Unlock the black hole<br>Cost: " +
        shortenCosts(new Decimal("1e4000")) +
        " EP";
      document.getElementById(
        "blackholeunlock"
      ).className = player.eternityPoints.gte("1e4000")
        ? "storebtn"
        : "unavailablebtn";
    }
  }
...
function updateDimensions() {
  if (document.getElementById("dimensions").style.display == "block") {
    if (
      ...
      dimensionTabDisplay();
    if (document.getElementById("infinitydimensions").style.display == "block")
      updateInfinityDimensions();
    if (document.getElementById("timedimensions").style.display == "block")
      updateTimeDimensions();
    if (document.getElementById("pdims").style.display == "block")
      paradoxDimDisplay();
    if (document.getElementById("metadimensions").style.display == "block")
      updateMetaDimensions();
    if (document.getElementById("emperordimensions").style.display == "block")
...
function updatePerformanceTicks() {
  if (player.aarexModifications.performanceTicks)
    ...
    slider.min = 33; // normally the lowest option
...
function replicantiDisplay() {
  if (player.replicanti.unl) {
    ...
    var chanceDisplayEnding =
      isChanceAffordable() && player.infinityPoints.l < 1e10
        ? "<br>+1% Cost: " + shortenCosts(player.replicanti.chanceCost) + " IP"
        : "";
    document.getElementById("replicantichance").innerHTML =
      "Replicate " +
      (tmp.rep.freq
        ? "amount: " + shorten(tmp.rep.freq) + "x"
        : "chance: " +
          getFullExpansion(
            chance.gt(1e12) ? chance : Math.round(chance.toNumber())
          ) +
          "%") +
      chanceDisplayEnding;
    document.getElementById("replicantiinterval").innerHTML =
      "Interval: " +
      timeDisplayShort(Decimal.div(tmp.rep.interval, 100), true, 3) +
      (isIntervalAffordable()
        ? "<br>-> " +
          timeDisplayShort(Decimal.times(tmp.rep.interval, 9e-3), true, 3) +
          " Cost: " +
          shortenCosts(player.replicanti.intervalCost) +
          " IP"
        : "");
    var replGalName =
      player.replicanti.gal < 3e3
        ? "Max Replicanti galaxies"
        : (player.replicanti.gal < 58200 ? "Distant" : "Further") +
          " Replicated Galaxies";
    var replGalCostPortion =
      player.infinityPoints.l < 1e10
        ? "<br>+1 Cost: " + shortenCosts(getRGCost()) + " IP"
        : "";
    document.getElementById("replicantimax").innerHTML =
      replGalName +
      ": " +
      getFullExpansion(player.replicanti.gal) +
      (replGalOver > 1 ? "+" + getFullExpansion(replGalOver) : "") +
      replGalCostPortion;
    document.getElementById("replicantireset").innerHTML =
      (!tmp.ngp3l && player.achievements.includes("ng3p67")
        ...
        ? "Divide replicanti amount by " +
          shorten(Number.MAX_VALUE) +
          ", but get "
        : "Reset replicanti amount, but get ") +
      "1 free galaxy.<br>" +
      getFullExpansion(player.replicanti.galaxies) +
      (extraReplGalaxies ? "+" + getFullExpansion(extraReplGalaxies) : "") +
      " replicated galax" +
      (getTotalRG() == 1 ? "y" : "ies") +
      " created.";
    document.getElementById("replicantiapprox").innerHTML =
      tmp.ngp3 &&
      player.dilation.upgrades.includes("ngpp1") &&
      player.timestudy.studies.includes(192) &&
      player.replicanti.amount.gte(Number.MAX_VALUE) &&
      (!player.aarexModifications.nguspV || player.aarexModifications.nguepV)
        ? "Replicanti increases by " +
          (tmp.rep.est < Math.log10(2)
            ? "x2.00 per " +
              timeDisplayShort((Math.log10(2) / tmp.rep.est) * 10)
            : (tmp.rep.est.gte(1e4)
                ? shorten(tmp.rep.est) + " OoMs"
                : "x" + shorten(Decimal.pow(10, tmp.rep.est.toNumber()))) +
              " per second") +
          ".<br>" +
          "Replicate interval slows down by " +
          tmp.rep.speeds.inc.toFixed(3) +
          "x per " +
          getFullExpansion(Math.floor(tmp.rep.speeds.exp)) +
          " OoMs.<br>" +
          "(2x slower per " +
          getFullExpansion(
            Math.floor(
              (tmp.rep.speeds.exp * Math.log10(2)) /
                Math.log10(tmp.rep.speeds.inc)
            )
          ) +
          " OoMs)"
        : "Approximately " +
          timeDisplay(
            Math.max(
              (Math.log(Number.MAX_VALUE) - tmp.rep.ln) /
                tmp.rep.est.toNumber(),
              0
            ) * 10
          ) +
    ...
    document.getElementById(
      "replicantimax"
    ).className = player.infinityPoints.gte(getRGCost())
      ? "storebtn"
      : "unavailablebtn";
    document.getElementById(
      "replicantireset"
    ).className = canGetReplicatedGalaxy() ? "storebtn" : "unavailablebtn";
    document.getElementById("replicantireset").style.height =
      (player.achievements.includes("ngpp16") &&
      (tmp.ngp3l || !player.achievements.includes("ng3p67"))
        ? 90
        : 70) + "px";
    ...
    document.getElementById(
      "replicantiunlock"
    ).className = player.infinityPoints.gte(
      player.galacticSacrifice != undefined &&
        player.tickspeedBoosts == undefined
        ? 1e80
        : 1e140
...
function initialTimeStudyDisplay() {
  document.getElementById("11desc").textContent =
    "Currently: " + shortenMoney(tsMults[11]()) + "x";
  document.getElementById("32desc").textContent =
    "You gain " +
    getFullExpansion(tsMults[32]()) +
    "x more infinities (based on Dimension Boosts)";
  document.getElementById("51desc").textContent =
    "You gain " +
    shortenCosts(player.aarexModifications.newGameExpVersion ? 1e30 : 1e15) +
    "x more IP";
  document.getElementById("71desc").textContent =
    "Currently: " +
    shortenMoney(tmp.sacPow.pow(0.25).max(1).min("1e210000")) +
    "x";
  document.getElementById("72desc").textContent =
    "Currently: " +
    shortenMoney(tmp.sacPow.pow(0.04).max(1).min("1e30000")) +
    "x";
  document.getElementById("73desc").textContent =
    "Currently: " +
    shortenMoney(tmp.sacPow.pow(0.005).max(1).min("1e1300")) +
    "x";
  document.getElementById("82desc").textContent =
    "Currently: " +
    shortenMoney(
      Decimal.pow(1.0000109, Decimal.pow(player.resets, 2)).min(
        player.meta == undefined ? 1 / 0 : "1e80000"
      )
    ) +
    "x";
  ...
  document.getElementById("93desc").textContent =
    "Currently: " +
    shortenMoney(Decimal.pow(player.totalTickGained, 0.25).max(1)) +
    "x";
  ...
  document.getElementById("161desc").textContent =
    shortenCosts(
      Decimal.pow(
        10,
        (player.galacticSacrifice ? 6660 : 616) *
          (player.aarexModifications.newGameExpVersion ? 5 : 1)
      )
  ...
  document.getElementById("162desc").textContent =
    shortenCosts(
      Decimal.pow(
        10,
        (player.galacticSacrifice ? 234 : 11) *
          (player.aarexModifications.newGameExpVersion ? 5 : 1)
      )
    ) + "x multiplier on all Infinity Dimensions";
  ...
  document.getElementById("193desc").textContent =
    "Currently: " +
    shortenMoney(Decimal.pow(1.03, getEternitied()).min("1e13000")) +
    "x";
  document.getElementById("212desc").textContent =
    "Currently: " + ((tsMults[212]() - 1) * 100).toFixed(2) + "%";
  document.getElementById("214desc").textContent =
    ...
    shortenMoney(
      tmp.sacPow
        .pow(8)
        .min("1e46000")
        .times(tmp.sacPow.pow(1.1))
        .div(tmp.sacPow)
        .max(1)
        .min(new Decimal("1e125000"))
    ) +
    "x";
  document.getElementById("metaCost").textContent = shortenCosts(
    getMetaUnlCost()
  );
}
...
function eternityChallengeUnlockDisplay() {
  ...
  if (player.etercreq !== 1)
    document.getElementById("ec1unl").innerHTML =
      ...
      (ECTimesCompleted("eterc1") + 1) * ec1Mult +
...
function mainTimeStudyDisplay() {
  ...
  document.getElementById("dilstudy1").innerHTML =
    "Unlock time dilation" +
    (player.dilation.studies.includes(1)
      ? ""
      : "<span>Requirement: 5 EC11 and EC12 completions and " +
        getFullExpansion(getDilationTotalTTReq()) +
        " total theorems") +
    "<span>Cost: " +
    getFullExpansion(5e3) +
    " Time Theorems";
  if (tmp.ngp3) {
    var ts232display = tmp.ts232 * 100 - 100;
    document.getElementById("221desc").textContent =
      "Currently: " + shorten(Decimal.pow(1.0025, player.resets)) + "x";
    document.getElementById("227desc").textContent =
      "Currently: " + shorten(Math.pow(tmp.sacPow.max(10).log10(), 10)) + "x";
    document.getElementById("231desc").textContent =
      "Currently: " +
      shorten(Decimal.pow(Math.max(player.resets, 1), 0.3)) +
      "x more power";
    document.getElementById("232desc").textContent =
      "Currently: " +
      (ts232display >= 999.95
        ? getFullExpansion(Math.floor(ts232display))
        : ts232display.toFixed(1)) +
      "%";
  }
}
...
function getReplMult(next) {
  let exp = 2;
  if (player.galacticSacrifice !== undefined)
    exp = Math.max(2, Math.pow(player.galaxies, 0.4));
  if (player.boughtDims) {
...
function upgradeReplicantiChance() {
  if (
    ...
    player.replicanti.chance =
      Math.round(player.replicanti.chance * 100 + 1) / 100;
  ...
  if (
    ...
    if (player.replicanti.interval < 1)
      player.replicanti.intervalCost = Decimal.pow(
        "1e800",
        1 / player.replicanti.interval
      );
    else
      player.replicanti.intervalCost = player.replicanti.intervalCost.times(
        1e10
      );
    if (!isIntervalAffordable())
      player.replicanti.interval =
        player.timestudy.studies.includes(22) || player.boughtDims ? 1 : 50;
    if (player.currentEternityChall == "eterc8") player.eterc8repl -= 1;
    document.getElementById("eterc8repl").textContent =
      "You have " + player.eterc8repl + " purchases left.";
  }
  ...
  if (offset > 0) {
    if (inQC(5)) return player.replicanti.galCost.pow(Math.pow(1.2, offset));
      ...
      if (player.replicanti.gal + offset > 99)
        increase +=
          (offset - Math.max(99 - player.replicanti.gal, 0)) *
          (25 *
            (offset -
              Math.max(99 - player.replicanti.gal, 0) +
              Math.max(player.replicanti.gal, 99) * 2) -
            4725);
      if (player.replicanti.gal + offset > 399) {
        if (player.exdilation != undefined)
          for (
            var g = Math.max(player.replicanti.gal, 399);
            g < player.replicanti.gal + offset;
            g++
          )
        ...
        if (player.meta != undefined) {
          ...
          if (player.masterystudies != undefined)
            if (player.masterystudies.includes("t266")) isReduced = true;
          if (isReduced) {
            increase +=
              (offset - Math.max(399 - player.replicanti.gal, 0)) *
              (1500 *
                (offset -
                  Math.max(399 - player.replicanti.gal, 0) +
                  Math.max(player.replicanti.gal, 399) * 2) -
                1183500);
            if (player.replicanti.gal + offset > 2998)
              increase +=
                (offset - Math.max(2998 - player.replicanti.gal, 0)) *
                (5e3 *
                  (offset -
                    Math.max(2998 - player.replicanti.gal, 0) +
                    Math.max(player.replicanti.gal, 2998) * 2) -
                  29935e3);
            if (player.replicanti.gal + offset > 58198)
              increase +=
                (offset - Math.max(58199 - player.replicanti.gal, 0)) *
                (1e6 *
                  (offset -
                    Math.max(58199 - player.replicanti.gal, 0) +
                    Math.max(player.replicanti.gal, 58199) * 2) -
                  58199e6);
          } else
            for (
              var g = Math.max(player.replicanti.gal, 399);
...
function replicantiGalaxy() {
  ...
  if (tmp.ngp3l || !player.achievements.includes("ng3p67"))
    player.replicanti.amount = Decimal.div(
      player.achievements.includes("r126") ? player.replicanti.amount : 1,
      Number.MAX_VALUE
    ).max(1);
  galaxyReset(0);
}
...
function canGetReplicatedGalaxy() {
  return (
    player.replicanti.galaxies < getMaxRG() &&
...
function canAutoReplicatedGalaxy() {
  return (
    speedrunMilestonesReached >= 20 || !player.timestudy.studies.includes(131)
  );
}
...
function updateExtraReplGalaxies() {
  ...
  if (extraReplGalaxies > 325)
    extraReplGalaxies =
      (Math.sqrt(0.9216 + 0.16 * (extraReplGalaxies - 324)) - 0.96) / 0.08 +
      324;
  if (tmp.ngp3) {
    let expData = {
      normal: 1 / 3,
    ...
    let expVarName =
      (player.masterystudies.includes("t362") ? "ts362" : "") +
      (tmp.ngp3l ? "legacy" : "");
    if (expVarName == "") expVarName = "normal";
    let exp = expData[expVarName];
    if (!tmp.ngp3l && player.masterystudies.includes("t412")) exp = 0.5;

    tmp.pe = Math.pow(tmp.qu.replicants.quarks.add(1).log10(), exp);
    tmp.pe *= tmp.ngp3l
      ? 0.67 * (player.masterystudies.includes("t412") ? 1.25 : 1)
      : 0.8;
...
function updateMilestones() {
  var moreUnlocked =
    tmp.ngp3 && (player.dilation.upgrades.includes("ngpp3") || quantumed);
...
function replicantiGalaxyAutoToggle() {
  ...
  document.getElementById("replicantiresettoggle").textContent =
    "Auto galaxy: " +
    (player.replicanti.galaxybuyer ? "ON" : "OFF") +
    (!canAutoReplicatedGalaxy() ? " (disabled)" : "");
}
...
function toggleCrunchMode(freeze) {
  if (player.autoCrunchMode == "amount") {
    ...
    document.getElementById("togglecrunchmode").innerHTML =
      "Auto crunch mode: replicated galaxies";
    document.getElementById("limittext").innerHTML =
      "Replicanti galaxies needed for crunch:";
    document.getElementById("maxReplicantiCrunchSwitchDiv").style.display =
      "inline";
  } else {
    player.autoCrunchMode = "amount";
    document.getElementById("togglecrunchmode").textContent =
    ...
    if (
      !freeze &&
      player.autobuyers[11].priority.toString().toLowerCase() == "max"
    ) {
...
function toggle_mod(id) {
  ...
  if (modes.ngp3ex != ngp3ex) {
    if (ngp3ex)
      $.notify(
        "In the deep depths of all Big Rips, a space crystal is impending you with more challenges..."
      );
    modes.ngp3ex = ngp3ex;
  }
}
    ...
    if (type == metaSave.current) {
      ...
      if (infiniteDetected) {
        if (document.getElementById("welcome").style.display != "flex")
          document.getElementById("welcome").style.display = "flex";
        document.getElementById("welcomeMessage").innerHTML =
          "Because you imported a save that has an Infinite bug in it, saving is disabled. Most functionality of this save is disabled to prevent further damage. It is highly recommended that you report this bug occurrence to the #bugs_and_glitches channel on the Discord server so the bug can be looked into and fixed. It is not recommended to modify this save, as it will be hard reset after you switch saves or refresh the game.";
      }
      startInterval();
    } else if (type == "new") {
      var newSaveId = 1;
...
function canBreakInfinity() {
  if (player.aarexModifications.ngexV)
    return player.challenges.length == getTotalNormalChallenges() + 1;
  return (
    player.autobuyers[11] % 1 != 0 && player.autobuyers[11].interval <= 100
  );
}
...
function breakInfinity() {
  ...
  if (player.galacticSacrifice)
    if (
      ...
      document.getElementById("quantumBlock").style.display = player.break
        ? ""
        : "none";
      document.getElementById("sacpos").className = player.break
        ? "quantumpos"
        : "eterpos";
      document.getElementById("galaxyPoints2").className = player.break
        ? "QK"
        : "EP";
    }
}
...
function gainedInfinityPoints(next) {
  ...
  if (player.infinityUpgradesRespecced == undefined)
    ...
    var ret = player.money
      .div(Number.MAX_VALUE)
      .pow((2 * (1 - Math.log10(2))) / Decimal.log10(Number.MAX_VALUE))
      .times(getIPMult());
  if (player.timestudy.studies.includes(41))
    ret = ret.times(
      Decimal.pow(tsMults[41](), player.galaxies + player.replicanti.galaxies)
    );
  ...
  if (player.boughtDims) {
    ret = ret.times(
      Decimal.pow(
        Math.max(1e4 / player.thisInfinityTime),
        player.timestudy.ers_studies[5] + (next == 5 ? 1 : 0)
      )
    );
    ret = ret.times(
      Decimal.pow(
        player.thisInfinityTime / 10,
        player.timestudy.ers_studies[6] + (next == 6 ? 1 : 0)
      )
    );
  }
...
function getIPMult() {
  ...
  if (player.galacticSacrifice && player.tickspeedBoosts == undefined) {
    ...
    if (player.achievements.includes("r55"))
      mult = mult.times(
        Math.min(Math.log10(Math.max(6000 / player.bestInfinityTime, 10)), 10)
      );
    if (player.achievements.includes("r41"))
      mult = mult.times(
        Math.pow(Math.log10(Math.max(player.spreadingCancer, 10)), 0.05)
      );
...
function gainedEternityPoints() {
  var ret = Decimal.pow(
    ...
    player.infinityPoints.plus(gainedInfinityPoints()).e /
      (player.achievements.includes("ng3p23") ? 307.8 : 308) -
      0.7
  )
    .times(player.epmult)
    .times(kongEPMult);
...
function setAchieveTooltip() {
  ...
  let noPointAchieve = document.getElementById(
  ...
  if (!tmp.ngp3l) {
    if (player.galacticSacrifice !== undefined)
      thisisReward.push(
        "g23 is more effective based on your best IP in dilation"
      );
    if (tmp.newNGP3E)
      thisisReward.push(
        "You gain 3x more DT while you produce less than " +
          shortenCosts(1e100) +
          " DT/second"
      );
  }
  ...
  if (!tmp.ngp3l && (tmp.ngp3 || tmp.newNGP3E))
    onlywarReward.push("You get 2x more DT");
  if (player.aarexModifications.nguspV !== undefined)
    onlywarReward.push(
      "you can auto-buy dilation upgrades every second if you have at least " +
  ...
  alot.setAttribute(
    "ach-tooltip",
    "Buy a single Second Dimension." +
      (player.aarexModifications.ngmX > 3
        ? " Reward: You gain 100x more Time Shards."
        : "")
  );
  ...
  ndial.setAttribute(
    "ach-tooltip",
    "Have exactly 99 Eighth Dimensions. Reward: Eighth Dimensions are 10% stronger" +
      (player.tickspeedBoosts == undefined
        ? "."
        : " and you gain more GP based on your Eighth Dimensions and your Tickspeed Boosts.")
  );
  ...
  gal.setAttribute(
    "ach-tooltip",
    "Buy an Antimatter Galaxy. " +
      (player.aarexModifications.ngmX > 3
        ? "Reward: Upon a Time Dimension Boost, your Dimension Boosts don’t reset unless you have more Time Dimension Boosts than your Dimension Boosts."
        : "")
  );
  doubleGal.setAttribute(
    "ach-tooltip",
    "Buy 2 Antimatter Galaxies. " +
      (player.tickspeedBoosts !== undefined
        ? "Reward: Upon a Tickspeed Boost, your Dimension Boosts" +
          (player.aarexModifications.ngmX > 3
            ? " and Time Dimension Boosts"
            : "") +
          " don’t reset unless you have more Tickspeed Boosts than five times your Antimatter Galaxies minus eight."
        : "") +
      (player.aarexModifications.ngmX > 3
        ? " You start with 3 Time Dimension Boosts."
        : "")
  );
  noPointAchieve.setAttribute(
    ...
    "Buy a single First Dimension when you have over " +
      ...
      (player.tickspeedBoosts == undefined
        ? "."
        : " and you can max buy Dimension and Tickspeed Boosts.")
  );
  infinity.setAttribute(
    "ach-tooltip",
    "Big Crunch for the first time. Reward: Start with 100 antimatter" +
      (player.galacticSacrifice
        ? " and always have at least 10x lower dimension costs."
        : ".")
  );

  //ACHIEVEMENT ROW 3
  claustrophobic.setAttribute(
    "ach-tooltip",
    "Big Crunch with just 1 Antimatter Galaxy. Reward: Reduce the starting tick interval by 2%" +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " and keep galaxy upgrades on infinity."
        : ".")
  );
  nerf.setAttribute(
    "ach-tooltip",
  ...
  didnt.setAttribute(
    "ach-tooltip",
    "Big Crunch without having any 8th Dimensions. Reward: Dimensions 1-7 are 2" +
      (player.galacticSacrifice ? "x" : "%") +
      " stronger."
  );
  fast.setAttribute(
    "ach-tooltip",
    "Big Crunch in under 2 hours. Reward: Start with " +
      shortenCosts(1e3) +
      " antimatter" +
      (player.galacticSacrifice
        ? " and get a multiplier to galaxy points based on fastest infinity (5 hours / x, 10x softcap)."
        : ".")
  );
  lot.setAttribute(
    "ach-tooltip",
    "Get at least 10 infinities." +
      (player.galacticSacrifice
        ? " Reward: " +
          (player.tickspeedBoosts == undefined
            ? "Start Infinities with galaxy points based on your infinities (x^2/100)."
            : " Keep galaxy upgrades on infinity.")
        : "")
  );
  ...
  cancer.setAttribute(
    "ach-tooltip",
    "Buy ten Galaxies in total while using cancer notation." +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " Reward: Gain a multiplier to IP based on the number of galaxies bought in cancer notation."
        : "")
  );
  zero.setAttribute(
    "ach-tooltip",
    "Big Crunch without Dimension shifts, boosts or galaxies in a challenge. Reward: Dimensions 1-4 are 25% stronger" +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " and you get 1.25x more IP."
        : ".")
  );
  potato.setAttribute(
    "ach-tooltip",
  ...
  anti.setAttribute(
    "ach-tooltip",
    "Complete all the challenges. Reward: All dimensions are 10% stronger" +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " and the tickspeed cost is also reduced based on dimension cost reduction."
        : ".")
  );

  //ACHIEVEMENT ROW 5
  forever.setAttribute(
    "ach-tooltip",
    "Big Crunch in 1 minute or less. Reward: Start with " +
      shortenCosts(1e10) +
      " antimatter" +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " and gain a multiplier to IP based on your best infinity time."
        : ".")
  );
  many.setAttribute(
    "ach-tooltip",
    "Complete the Second Dimension Autobuyer challenge in 3 minutes or less. Reward: All dimensions are stronger in the first 3 minutes of an Infinity" +
      (player.tickspeedBoosts == undefined
        ? "."
        : " and you gain 1% of GP gained on Galaxy Sacrifice per second.")
  );
  is.setAttribute(
    "ach-tooltip",
    "Complete the Tickspeed Autobuyer challenge in 3 minutes or less.  Reward: The multiplier per-10 dimensions " +
      (player.tickspeedBoosts != undefined
        ? "is boosted based on your best time of Tickspeed Autobuyer challenge."
        : player.galacticSacrifice
        ? "is raised to the power of ^1.0666."
        : " is increased by 1%.")
  );
  limitBreak.setAttribute(
    "ach-tooltip",
    "Break Infinity." +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " Reward: Gain a multiplier to IP based on galaxies."
        : "")
  );
  ...
  oh.setAttribute(
    ...
    "Reach " +
      ...
      " IP per minute." +
      (player.galacticSacrifice && player.tickspeedBoosts == undefined
        ? " Reward: Gain a multiplier to GP based on the logarithm of your IP."
        : "")
  );
  mil.setAttribute(
    "ach-tooltip",
  ...
  right.setAttribute(
    "ach-tooltip",
    "Complete the Third Dimension Autobuyer challenge in 10 seconds or less. Reward: First Dimensions are 5" +
      (player.galacticSacrifice ? "x" : "0%") +
      " stronger."
  );
  infchall.setAttribute(
    "ach-tooltip",
    "Complete an Infinity Challenge." +
      (player.galacticSacrifice
        ? " Reward: Galaxies and " +
          (player.tickspeedBoosts === undefined ? "g11" : "Tickspeed Boosts") +
          " are more effective based on IC's completed."
        : "")
  );
  ...
  not.setAttribute(
    "ach-tooltip",
    "Big Crunch with only a single first Dimension without Dimension Boosts, Shifts or Galaxies while in Automatic Galaxies Challenge. Reward: First Dimensions are " +
      (player.galacticSacrifice ? 909 : 3) +
      " times stronger."
  );
  blink.setAttribute(
    "ach-tooltip",
    "Big Crunch in under 200 milliseconds. Reward: Start with " +
      formatValue(player.options.notation, 2e25, 0, 0) +
      " antimatter, and all dimensions are stronger in the first 300 milliseconds of this Infinity."
  );
  cant.setAttribute(
    "ach-tooltip",
    "Get all Dimension multipliers over " +
      shortenCosts(1e308) +
      ". Reward: All dimensions are 10" +
      (player.galacticSacrifice ? "x" : "%") +
      " stronger."
  );
  newDim.setAttribute(
    "ach-tooltip",
    "Unlock the 4th Infinity Dimension." +
      (player.boughtDims
        ? ""
  ...
  tables.setAttribute(
    "ach-tooltip",
    "Get 8th Dimension multiplier to be highest, 7th Dimension multiplier second highest etc. Reward: Each dimension gains a boost proportional to their tier (8th dimension gets 8" +
      (player.galacticSacrifice ? "0" : "") +
      "%, 7th gets 7" +
      (player.galacticSacrifice ? "0" : "") +
      "%, etc.)"
  );
  ...
  twomillion.setAttribute(
    "ach-tooltip",
    "Infinity 2000000 times. Reward: Infinities that are more than 5 seconds long give 250 more infinities" +
      (player.galacticSacrifice
        ? ", and you gain an additive +249 infinites per crunch after multipliers"
        : "") +
      "."
  );
  ...
  overdrive.setAttribute(
    ...
    "Big Crunch with " +
      shortenCosts(1e300) +
      " IP/min. Reward: Gain an additonal 4x multiplier to IP."
  );
  minute.setAttribute(
    "ach-tooltip",
    "Reach " +
  ...
  hell.setAttribute(
    "ach-tooltip",
    "Get the sum of Infinity Challenge times under 5 seconds." +
      (player.boughtDims
        ? " Reward: Sacrifice is again slightly stronger."
        : "")
  );
  zerodeg.setAttribute(
    "ach-tooltip",
    "Unlock the 8th Infinity Dimension." +
      (player.boughtDims
        ? " Reward: Normal Dimensions are multiplied by the amount of 8th Infinity Dimensions you have."
        : "")
  );
  ...
  costco.setAttribute(
    "ach-tooltip",
    "Bulk buy 750 dimension boosts at once. Reward: Dimension boosts are " +
      (player.boughtDims
        ? "cheaper based on EP."
        : "1% more powerful (to normal dims).")
  );
  mile.setAttribute(
    "ach-tooltip",
    "Get " +
      (tmp.ngp3 ? "100 eternities milestone." : "all eternity milestones.")
  );
  swarm.setAttribute(
    "ach-tooltip",
    "Get 10 replicanti galaxies in 15 seconds." +
      (player.boughtDims
        ? " Reward: Unlock the ability to control replicanti galaxy power."
        : "")
  );
  inftime.setAttribute(
    "ach-tooltip",
    player.boughtDims
      ? "Eternity without buying dimensions 1-7. Reward: Time Dimensions gain a multiplier based on the eighth root of eighth dimensions."
      : "Get 308 tickspeed upgrades (in one eternity) from Time Dimensions. Reward: Time dimensions are affected slightly more by tickspeed."
  );
  guide.setAttribute(
    "ach-tooltip",
    player.boughtDims
      ? "Reach " +
          ...
          " replicanti. Reward: Replicanti increases faster the more you have."
      : "Eternity with less than 10 infinities."
  );
  nine.setAttribute(
    "ach-tooltip",
    "Eternity with exactly 9 replicanti." +
      (player.boughtDims
        ? " Reward: Replicanti multiplier to ID is 9% stronger (after time studies)."
        : "")
  );
  ...
  dawg.setAttribute(
    ...
    "Have all your past 10 infinities be at least " +
      shortenMoney(Number.MAX_VALUE) +
      " times higher IP than the previous one. Reward: Your antimatter doesn't reset when buying a Dimension Boost or Galaxy."
  );
  ...
  infiniteIP.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e30008")) +
      " IP." +
      (player.galacticSacrifice == undefined ||
      player.tickspeedBoosts != undefined
        ? ""
        : " Reward: Your total galaxies boost Galaxy points gain more.")
  );
  fiveMore.setAttribute(
    "ach-tooltip",
    "Complete 50 unique eternity challenge tiers." +
      (player.galacticSacrifice !== undefined
        ? " Reward: Divide Infinity Dimension costs based on the multiplier of g11."
        : "")
  );
  newI.setAttribute(
    "ach-tooltip",
    "Eternity in under 200ms." +
      (player.galacticSacrifice !== undefined
        ? " Reward: The Dimension Boosts boost to Galaxy points gain is buffed."
        : "")
  ); // by how much?
  eatass.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(1e100) +
      " IP without any infinities or first dimensions. Reward: Gain an IP multiplier based on time spent in this Infinity."
  );
  layer.setAttribute(
    "ach-tooltip",
    "Reach " +
      ...
      " EP." +
      (player.galacticSacrifice !== undefined
        ? " Reward: The Galaxy boost to Galaxy points gain is buffed."
        : "")
  ); // by how much?
  fkoff.setAttribute(
    "ach-tooltip",
  ...
  potato3.setAttribute(
    ...
    "Get more than " +
      ...
      " ticks per second." +
      (player.galacticSacrifice !== undefined
        ? " Reward: The Galaxy boost to Galaxy points gain is buffed further."
        : "")
  ); // by how much?
  infstuff.setAttribute(
    ...
    "Reach " +
      ...
      " IP without buying IDs or IP multipliers. Reward: You start eternities with all Infinity Challenges unlocked and completed" +
      (player.meta
        ? ", and your Infinity gain is multiplied by dilated time^(1/4)."
        : ".")
  );
  when.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal(tmp.ngex ? "1e15000" : "1e20000")) +
  ...
  thisis.setAttribute(
    ...
    "Reach " +
      ...
      (thisisReward != "" ? " Reward: " + thisisReward + "." : "")
  );

  //NGUD ACHIEVEMENT ROW (13.5)
  stillamil.setAttribute(
    "ach-tooltip",
    "Reach " + shortenCosts(1e6) + " black hole power."
  );
  out.setAttribute(
    "ach-tooltip",
    "Get more than " +
      ...
      " ex-dilation." +
      (player.aarexModifications.nguspV !== undefined
        ? " Reward: You can equally distribute ex-dilation to all repeatable dilation upgrades."
        : "")
  );
  ridNGud.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e20000")) +
      " IP without any time studies or dilation upgrades while dilated."
  );
  ...
  onlywar.setAttribute(
    ...
    "Reach " +
      shortenMoney(new Decimal("1e40000")) +
      " EP." +
      (onlywarReward != "" ? " Reward: " + onlywarReward + "." : "")
  );
  thecap.setAttribute(
    "ach-tooltip",
    "Get " +
      shortenDimensions(1e12) +
  ...
  neverenough.setAttribute(
    ...
    "Reach " +
      ...
      " replicanti. Reward: You unlock the option to buy the maximum Replicanti Galaxies available."
  );
  harmony.setAttribute(
    "ach-tooltip",
    player.meta
      ? "Have at least 700 normal, replicanti, and free dilated galaxies. Reward: Galaxies are 0.1% stronger."
      : "Get the same number (at least 300) of normal, replicanti, and free galaxies."
  );
  ...
  let seriously = document.getElementById(
    "Seriously, I already got rid of you."
  );
  ...
  let oppose = document.getElementById(
    "I rather oppose the theory of everything"
  );
  ...
  if (!tmp.ngp3l) {
    ...
    willenoughReward.push(
      "keep all your replicanti upgrades on Eternity only when you start a normal Eternity run"
    );
  }
  if (player.aarexModifications.ngudpV && !player.aarexModifications.ngumuV)
    willenoughReward.push("keep Black Hole Dimensions on Quantum");
  willenoughReward = wordizeList(willenoughReward, true);

  //ACHIEVEMENT ROW 15
  notenough.setAttribute(
    "ach-tooltip",
    "Reach " +
      shorten(Number.MAX_VALUE) +
      " meta-antimatter." +
      (tmp.ngp3l
        ? ""
        : " Reward: You produce more dilated time based on your normal galaxies. You gain more Tachyon particles based on your replicated galaxies.")
  );
  hadron.setAttribute(
    "ach-tooltip",
    "Have colored quarks, but have no color charge." +
      (tmp.ngp3l ? "" : " Reward: Quantum worth boosts all Meta Dimensions.")
  );
  old.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(getOldAgeRequirement()) +
      " antimatter." +
      (tmp.ngp3l
        ? ""
        : " Reward: Get a multiplier to the 1st Meta Dimension based on total antimatter.")
  );
  rid.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e400000")) +
      " IP while dilated without having studies and electrons. Reward: Generate time theorems based on your best-ever tachyon particles."
  );

  //ACHIEVEMENT ROW 16
  special.setAttribute(
    "ach-tooltip",
    "Quantum in under 5 seconds." +
      (tmp.ngp3l
        ? ""
        : " Reward: Start with all Infinity Dimensions unlocked if you have at least 25 eternities.")
  );
  memories.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e1700")) +
      " MA without ever buying 5th-8th Normal Dimensions or having more than 4 Dimension Boosts in this quantum." +
      (tmp.ngp3l
        ? ""
        : " Reward: The 4 RG upgrade is stronger based on your Meta-Dimension Boosts.")
  );
  squared.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e1500")) +
      " MA with exactly 8 meta-dimension boosts." +
      (tmp.ngp3l
        ? ""
        : " Reward: Get a multiplier to the 8th Meta Dimension based on your 1st Meta Dimension.")
  );
  morals.setAttribute(
    "ach-tooltip",
    "Quantum without any meta-dimension boosts." +
      (tmp.ngp3l
        ? ""
        : " Reward: Meta-Dimension Boosts boost itself at a reduced rate.")
  );
  seriously.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e354000")) +
      " IP without having time studies while dilated and running QC2." +
      (tmp.ngp3l
        ? ""
        : " Reward: The Eternity Points boost to Quark gain is 1% stronger.")
  );

  //QUANTUM SPEEDRUNS
  tfms.setAttribute(
    "ach-tooltip",
    "Reward: Start with " + shortenCosts(1e13) + " eternities."
  );
  tms.setAttribute(
    "ach-tooltip",
    "Reward: Start with " + shortenCosts(1e25) + " meta-antimatter on reset."
  );
  tfms2.setAttribute(
    "ach-tooltip",
    "Reward: Start with " +
      shortenCosts(1e100) +
      " dilated time and dilated time only resets on quantum."
  );
  ...
  internal.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(new Decimal("1e333")) +
      " MA without having 2nd Meta Dimensions and Meta-Dimension Boosts." +
      (tmp.ngp3l
        ? ""
        : " Reward: 1st Meta Dimensions are stronger based on meta antimatter.")
  );
  truth.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 7.88e13)) +
      " antimatter without having completed any paired challenges."
  );
  cantGet.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 6.2e11)) +
      " antimatter in Eternity Challenge 11."
  );
  noDil.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 2e6)) +
      " replicanti without having Tachyon particles. Reward: You start Quantums with the square root of your best TP as your Tachyon particle amount."
  );
  dontWant.setAttribute(
    "ach-tooltip",
    "Reach " +
      shorten(Decimal.pow(Number.MAX_VALUE, 1000)) +
      " IP while dilated, in QC2, and without having studies and First Dimensions during your current Eternity."
  );

  //ACHIEVEMENT ROW 18
  notrelative.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Decimal.pow(10, 411)) +
      " dilated time without gaining tachyon particles." +
      (tmp.ngp3l
        ? ""
        : " Reward: You gain more DT based on the amount of Nanofield rewards.")
  );
  error404.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Decimal.pow(10, 1.6e12)) +
      " antimatter while having only the 1st Dimensions of each type of Dimension and at least 2 normal galaxies."
  );
  ie.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Decimal.pow(10, 8e6)) +
      " antimatter in a paired challenge with the PC6+8 combination."
  );
  wasted.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(11e6) +
      " TT without having TT generation, keeping your previous TT, and respeccing studies. Reward: While you have less than 1 hour worth of TT production, you generate 10x as much TT."
  );
  stop.setAttribute(
    "ach-tooltip",
    "Get the replicanti reset requirement to " +
      shorten(Decimal.pow(10, 125e5)) +
      ". Reward: Getting a normal replicant manually doesn't reset your replicanti, and can be automated."
  );
  dying.setAttribute(
    "ach-tooltip",
    "Reach " +
      shorten(Decimal.pow(10, 2.75e5)) +
      " IP while dilated, in PC6+8, and without having studies." +
      (tmp.ngp3l
        ? ""
        : " Reward: Branches are faster based on your Meta-Dimension Boosts.")
  );
  ...
  gofast.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Decimal.pow(10, 1185)) +
      " EP first and then square your EP by disabling dilation while big ripped."
  );
  immunity.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Decimal.pow(10, 8e7)) +
      " antimatter with one normal galaxy while in Eternity Challenge 7 and big ripped." +
      (tmp.ngp3l ? "" : " Reward: Infinite Time is 3% stronger.")
  );
  notSmart.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(1e215) +
      " Time Shards without having time study 11 while big ripped." +
      (tmp.ngp3l
        ? ""
        : " Reward: Meta Dimensions get a multiplier based on time shards.")
  );
  soLife.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 3.5e5)) +
      " IP in Big Rip while dilated, with no EP multiplier upgrades and time studies."
  );
  ...
  finite.setAttribute(
    ...
    "Get " +
      shortenCosts(1e33) +
      " Space Shards without Breaking Eternity in this Ghostify." +
      (tmp.ngp3l
        ? ""
        : " Reward: Outside of Big Rips, Tree Upgrades are 10% stronger. In Big Rips, 8th Time Dimensions gain an small exponent boost based on your current Ghostify time.")
  );
  really.setAttribute(
    "ach-tooltip",
    "Undo the Big Rip with at least " +
      shortenCosts(Decimal.pow(10, 5000)) +
      " matter." +
      (tmp.ngp3l
        ? ""
        : " Reward: Buying Electron upgrades doesn't consume Meta-Dimension Boosts.")
  );
  willenough.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(
        Decimal.pow(10, player.aarexModifications.ngudpV ? 268435456 : 36000000)
      ) +
      " replicanti." +
      (willenoughReward != "" ? " Reward: " + willenoughReward + "." : "")
  );
  oppose.setAttribute(
    "ach-tooltip",
    "Become a ghost with at most 1x quantumed stat." +
      (tmp.ngp3l
        ? ""
        : " Reward: You gain more quarks based on your quantumed stat.")
  );
  pls.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 9.5e5)) +
      " IP in Big Rip while dilated, with no EP multiplier upgrades, time studies, and Break Eternity in this Ghostify. Reward: Each time you become a ghost, you gain " +
      shortenDimensions(2e3) +
      " galaxies worth of generated neutrinos, multiplied by your best-ever galaxy amount across all Big Rips."
  );

  //BRAVE MILESTONES
  bm1.setAttribute(
    "ach-tooltip",
    "Reward: Start Ghostifies with all Speedrun Milestones and all " +
      shorten(Number.MAX_VALUE) +
  ...
  bm10.setAttribute(
    "ach-tooltip",
    "Reward: Start Ghostifies with 10 of each Emperor Dimension, up to the 4th tier" +
      (player.aarexModifications.ngudpV
        ? ", and start Big Rips with the 3rd row of Eternity upgrades."
        : ".")
  );
  bm14.setAttribute(
    "ach-tooltip",
    "Reward: Start Ghostifies with " +
      shortenCosts(1e25) +
      " Quark Spins and Branches are faster based on spins (at least 10x)."
  );

  //ACHIEVEMENT ROW 21
  uc.setAttribute(
    "ach-tooltip",
    "Become a ghost with at least " +
      shortenCosts(Decimal.pow(10, 2.2e5)) +
      " EP without starting Eternity Challenge 10 while Big Ripped." +
      (tmp.ngp3l
        ? ""
        : " Reward: Meta-Dimension Boosts don't reset Meta Dimensions.")
  );
  mi.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Number.MAX_VALUE) +
      " infinities. Reward: You gain banked infinites and eternities when going Quantum or Big Ripping the universe."
  );
  wd.setAttribute(
    "ach-tooltip",
    "Get " +
      shortenCosts(Decimal.pow(10, 1e12)) +
      " Infinity Unstable Quarks for each Branch without Big Ripping in this Ghostify."
  );
  arent.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 1.8e6)) +
      " IP while dilated and Big Ripped and without having studies, EP mult upgrades, Tree Upgrades, and Break Eternity in this Ghostify." +
      (tmp.ngp3l
        ? ""
        : " Reward: Your 8th Tree Upgrade's level speeds up Nanofield.")
  );

  //ACHIEVEMENT ROW 22
  document
    .getElementById("Even Ghostlier than before")
    .setAttribute(
      "ach-tooltip",
      "Unlock Bosonic Lab." +
        (tmp.ngp3l
          ? ""
          : " Reward: The meta-antimatter effect uses your best meta-antimatter in your current Ghostify instead of your best in the current Quantum.")
    );
  ee.setAttribute(
    "ach-tooltip",
    "Get " + shorten(Number.MAX_VALUE) + " eternities."
  );
  oc.setAttribute(
    "ach-tooltip",
    "Become a ghost with at least " +
      shortenCosts(Decimal.pow(10, 3.75e5)) +
      " EP while Big Ripped with the Anti-Dilation modifier."
  );
  btco.setAttribute(
    "ach-tooltip",
    "Complete Paired Challenge 1 after getting " +
      shortenCosts(Decimal.pow(10, 1.65e9)) +
      " antimatter in Quantum Challenges 6 and 8."
  );
  tdc.setAttribute(
    "ach-tooltip",
    "Complete Eternity Challenge 11 with " +
      shortenCosts(Decimal.pow(10, 15500)) +
      " IP in a Paired Challenge with Quantum Challenges 6 and 8 combinations and Anti-Dilation modifier."
  );
  igu.setAttribute(
    "ach-tooltip",
    "Reach " +
      shortenCosts(Decimal.pow(10, 2.25e4)) +
      " IP while dilated and Big Ripped with Anti-Dilation modifier and without having studies, EP mult upgrades, Tree Upgrades, and Break Eternity in this Ghostify."
  );
  ...
  ghostliest.setAttribute(
    "ach-tooltip",
    "Get " +
      shorten(Math.pow(Number.MAX_VALUE, 1 / 4)) +
      " Ghostified stat. Reward: Ghostifies boost the gain of Ghost Particles at the reduced rate."
  );
  metae18.setAttribute(
    "ach-tooltip",
    "Get " +
      shortenCosts(Decimal.pow(10, 1e18)) +
      " antimatter. Reward: Distant Antimatter Galaxies scaling is 10% weaker."
  );
}
  ...
  if (tmp.ngp3) {
    ...
    if (!player.ghostify.ghostlyPhotons.unl)
      document.getElementById("gphUnl").textContent =
        ...
        shortenCosts(Decimal.pow(10, 6e9)) +
...
function calcSacrificeBoost() {
  ...
  if (
    ...
    (player.tickspeedBoosts !== undefined &&
      player.currentChallenge == "postc2")
  ) {
    pow = 0.01;
    ...
    ret = Decimal.pow(
      Math.max(player.firstAmount.e / 10, 1) /
        Math.max(player.sacrificed.e / 10, 1),
      pow
    ).max(1);
  } else
...
function updateAutobuyers() {
  ...
  if (player.autobuyers[10].interval <= 100) {
    if (
      player.infinityUpgradesRespecced != undefined &&
      !player.autobuyers[10].bulkBought
    )
      document.getElementById("buyerBtnGalaxies").innerHTML =
        "Buy bulk feature<br>Cost: " + shortenCosts(1e4) + currencyEnd;
...
function updateChallengeTimes() {
  ...
  setAndMaybeShow(
    "challengetimesum",
    tempcounter > 1,
    '"The sum of your completed challenge time records is "+timeDisplayShort(' +
      temp +
      ", false, 3)"
  );
  ...
  setAndMaybeShow(
    "infchallengetimesum",
    tempcounter > 1,
    '"The sum of your completed Infinity Challenge time records is " + timeDisplayShort(' +
      temp +
      ", false, 3)"
  );
...
function updateLastTenRuns() {
  ...
  for (var i = 0; i < 10; i++) {
    if (player.lastTenRuns[i][1].gt(0)) {
      ...
      var msg =
        "The infinity " +
        (i == 0 ? "1 infinity" : i + 1 + " infinities") +
        " ago took " +
        timeDisplayShort(player.lastTenRuns[i][0], false, 3);
      if (player.lastTenRuns[i][2]) {
        var split = player.lastTenRuns[i][2].split("challenge");
        if (split[1] == undefined)
          msg +=
            " in Infinity Challenge " + checkICID(player.lastTenRuns[i][2]);
...
function updateEterChallengeTimes() {
  ...
  setAndMaybeShow(
    ...
    tempcounter > 1,
    '"The sum of your completed Eternity Challenge time records is " + timeDisplayShort(' +
      temp +
      ", false, 3)"
  );
}
...
function eternity(force, auto, presetLoad, dilated) {
  ...
  if (
    ...
    (tmp.ngp3 ? player.dilation.active && tmp.qu.bigRip.active : false)
  )
  ...
  if (
    player.infinitiedBank >= 5000000000 &&
    !player.achievements.includes("r131")
  )
  ...
  if (getEternitied() >= 100 && !player.achievements.includes("r102"))
...
function resetReplicantiUpgrades() {
  ...
  ).times(player.galacticSacrifice !== undefined ? 1e75 : 1e135);
  player.replicanti.intervalCost = Decimal.pow(
    1e10,
  ...
  player.replicanti.galCost = new Decimal(
    player.galacticSacrifice != undefined ? 1e110 : 1e170
  );
}
...
function startChallenge(name) {
  ...
  if (player.options.challConf && name != "")
    if (
      !confirm(
        "You will start over with just your infinity upgrades, and achievements. You need to reach " +
          (name.includes("post") ? "a set goal" : "infinity") +
          " with special conditions. NOTE: The rightmost infinity upgrade column doesn't work on challenges."
      )
    )
      return;
  if (player.tickspeedBoosts !== undefined) player.tickspeedBoosts = 0;
...
function inNC(x, n) {
  if (x == 6) {
    if (
      ...
      player.aarexModifications.ngexV &&
      (player.currentChallenge == "" ||
        player.currentChallenge.indexOf("postc") == 0) &&
      player.currentChallenge != "postc1"
    )
      return true;
    if (
      n == 1 &&
      player.aarexModifications.ngexV &&
      player.currentChallenge == "challenge6"
    )
      return false;
  ...
  if (x == 0)
    return (
      player.currentChallenge == "" &&
      (!(player.aarexModifications.ngmX > 3) ||
        !player.galacticSacrifice.chall) &&
      inPxC(0)
    );
  return (
    player.currentChallenge == "challenge" + x ||
    (player.aarexModifications.ngmX > 3 &&
      player.galacticSacrifice.chall == x) ||
    inPxC(x)
  );
}
...
function updateNCVisuals() {
  ...
  if (
    ...
    inNC(9) ||
    inNC(12) ||
    ((inNC(5) || inNC(14) || chall == "postc4" || chall == "postc5") &&
      player.tickspeedBoosts == undefined) ||
    player.pSac !== undefined ||
    chall == "postc1" ||
    chall == "postc6" ||
...
function canUnlockEC(idx, cost, study, study2) {
  ...
  switch (idx) {
    case 1:
      if (
        getEternitied() >=
        (ECTimesCompleted("eterc1") ? ECTimesCompleted("eterc1") + 1 : 1) *
          ec1Mult
      )
        return true;
      break;
...
function startDilatedEternity(auto, shortcut) {
  ...
  if (!onActive && player.aarexModifications.dilationConf && !auto)
    if (
      !confirm(
        "Dilating time will start a new eternity, and all of your Dimension/Infinity Dimension/Time Dimension multiplier's exponents and tickspeed multiplier's exponent will be reduced to ^ 0.75. If you can eternity while dilated, you'll be rewarded with tachyon particles based on your antimatter and tachyon particles."
      )
    )
      return;
...
function dilates(x, m) {
  ...
  if (
    player.dilation.active &&
    m != 2 &&
    (m != "meta" || !player.achievements.includes("ng3p63") || !inQC(0))
  ) {
    e *= dilationPowerStrength();
    if (player.aarexModifications.newGameMult)
      e =
        0.9 + Math.min(player.dilation.dilatedTime.add(1).log10() / 1000, 0.05);
    if (
      player.exdilation != undefined &&
      !player.aarexModifications.ngudpV &&
      !player.aarexModifications.nguspV
    )
      e += exDilationBenefit() * (1 - e);
    if (player.dilation.upgrades.includes(9)) e *= 1.05;
    if (player.dilation.rebuyables[5])
      e +=
        0.0025 * (1 - 1 / Math.pow(player.dilation.rebuyables[5] + 1, 1 / 3));
    a = true;
  }
...
function getDilUpgCost(id) {
  ...
  if (ngpp) {
    ngpp = parseInt(ngpp);
    if (ngpp >= 3 && player.aarexModifications.nguspV !== undefined)
      cost = DIL_UPG_COSTS[id + "_usp"];
  }
  return cost;
}
...
function getRebuyableDilUpgCost(id) {
  ...
  if (id == 4 && player.galacticSacrifice !== undefined && !tmp.ngp3l)
    costGroup = DIL_UPG_COSTS.r4_ngmm;
  ...
  if (player.aarexModifications.nguspV) {
    ...
    if (player.meta != undefined && amount >= costGroup[2])
      return cost.times(
        Decimal.pow(
          costGroup[1],
          ((amount - costGroup[2] + 1) * (amount - costGroup[2] + 2)) / 4
        )
      );
    if (
      player.exdilation != undefined &&
...
function buyDilationUpgrade(pos, max, isId) {
  ...
  if (rebuyable) {
    ...
    player.dilation.rebuyables[id[1]] =
      (player.dilation.rebuyables[id[1]] || 0) + 1;
    ...
    if (
      player.aarexModifications.nguspV !== undefined &&
      !player.dilation.autoUpgrades.includes(id)
    )
    ...
    if (id == "ngpp6" && tmp.ngp3) {
      ...
      if (!quantumed) {
        $.notify(
          "Congratulations, you unlocked Mastery Studies! You can now click the 'mastery studies' button\nor the 'continue to mastery studies' button in the time studies menu to access it."
        );
        document.getElementById("welcomeMessage").innerHTML =
          "Congratulations for reaching the end-game of NG++. In NG+3, the game continues with a lot of new content starting at Mastery Studies. You can now either click the 'mastery studies' button or the 'continue to mastery studies' button in time studies to access it, which reveals a lot more time studies.";
      }
    }
  }
  if (max) return true;
...
function getTTGenPart(x) {
  x = x.max(1).log10();
  let y =
    player.aarexModifications.ngudpV && !player.aarexModifications.nguepV
      ? 73
      : 80;
...
function updateDilationUpgradeButtons() {
  for (var i = 0; i < DIL_UPGS.length; i++) {
    ...
    if (DIL_UPG_UNLOCKED[id] != unl) {
      ...
      document.getElementById("dil" + pos).parentElement.style.display = unl
        ? ""
        : "none";
    }
    if (unl)
      document.getElementById("dil" + pos).className =
        player.dilation.upgrades.includes(id) ||
        (id == "r2" && !canBuyGalaxyThresholdUpg())
          ? "dilationupgbought"
          : player.dilation.dilatedTime.gte(getDilUpgCost(id))
          ? "dilationupg"
          : "dilationupglocked";
  }
  ...
  document.getElementById("dil13desc").textContent =
    power > 3
      ? "Gain " + shorten(power) + "x more Tachyon Particles."
      : "Triple the amount of Tachyon Particles gained.";
  document.getElementById("dil31desc").textContent =
    "Currently: " +
    shortenMoney(player.dilation.dilatedTime.max(1).pow(1000).max(1)) +
    "x";
  document.getElementById("dil41desc").textContent =
    "Currently: " +
    shortenMoney(
      player.achievements.includes("ng3p44") &&
        player.timestudy.theorem / genSpeed < 3600
        ? genSpeed * 10
        : genSpeed
    ) +
    "/s";
  if (player.dilation.studies.includes(6)) {
    document.getElementById("dil51desc").textContent =
      "Currently: " + shortenMoney(getDil14Bonus()) + "x";
    document.getElementById("dil52desc").textContent =
      "Currently: " + shortenMoney(getDil15Bonus()) + "x";
    document.getElementById("dil54formula").textContent =
      "(log(x)^0.5" + (tmp.ngp3 ? ")" : "/2)");
    document.getElementById("dil54desc").textContent =
      "Currently: " + shortenMoney(getDil17Bonus()) + "x";
  }
  if (player.exdilation != undefined)
    document.getElementById("dil61desc").textContent =
      "Currently: " + shortenMoney(getD18Bonus()) + "x";
  if (isDilUpgUnlocked("ngusp2")) {
    document.getElementById("dil64desc").textContent =
      "Currently: +" +
      shortenMoney(getD21Bonus()) +
      " to exponent before softcap";
    document.getElementById("dil65desc").textContent =
      "Currently: " + shortenMoney(getD22Bonus()) + "x";
  }
  if (player.galacticSacrifice !== undefined) {
    document.getElementById("dil44desc").textContent =
      "Currently: +" + shortenMoney(getDil44Mult());
    document.getElementById("dil45desc").textContent =
      "Currently: " + shortenMoney(getDil45Mult()) + "x";
    if (player.dilation.studies.includes(6)) {
      document.getElementById("dil71desc").textContent =
        "Currently: ^" + shortenMoney(getDil71Mult());
      document.getElementById("dil72desc").textContent =
        "Currently: " + shortenMoney(getDil72Mult()) + "x";
    }
  }
}

function updateDilationUpgradeCost(pos, id) {
  if (id == "r2" && !canBuyGalaxyThresholdUpg())
    document.getElementById("dil" + pos + "cost").textContent = "Maxed out";
  else {
    let r = getDilUpgCost(id);
    if (id == "r3")
      r = formatValue(player.options.notation, getRebuyableDilUpgCost(3), 1, 1);
    else r = shortenCosts(r);
    document.getElementById("dil" + pos + "cost").textContent =
      "Cost: " + r + " dilated time";
  }
  if (id == "ngud1")
    document.getElementById("dil61oom").textContent = shortenCosts(
      new Decimal("1e1000")
    );
}
...
function gainDilationGalaxies() {
  if (player.dilation.dilatedTime.gte(player.dilation.nextThreshold)) {
    let thresholdMult = inQC(5)
      ? Math.pow(10, 2.8)
      : !canBuyGalaxyThresholdUpg()
      ? 1.35
      : 1.35 + 3.65 * Math.pow(0.8, getDilUpgPower(2));
    if (hasBosonicUpg(12)) {
      thresholdMult -= tmp.blu[12];
      if (!tmp.ngp3l && thresholdMult < 1.2)
        thresholdMult = 1.1 + 0.1 / Math.sqrt(2.2 - thresholdMult);
      else if (thresholdMult < 1.15)
        thresholdMult = 1.05 + 0.1 / (2.15 - thresholdMult);
    }
    if (player.exdilation != undefined)
      thresholdMult -= Math.min(0.1 * exDilationUpgradeStrength(2), 0.2);
    ...
    player.dilation.nextThreshold = Decimal.pow(
      thresholdMult,
      timesGained - thresholdGalaxies
    ).times(player.dilation.nextThreshold);
...
function getFreeGalaxyGainMult() {
  ...
  if (isNanoEffectUsed("dil_gal_gain"))
    galaxyMult *= tmp.nf.effects.dil_gal_gain;
  return galaxyMult;
}
...
function updateInfPower() {
  ...
  if (player.galacticSacrifice && player.pSac == undefined)
    document.getElementById(
      "infPowEffectPower"
    ).textContent = tmp.infPowExp.toFixed(2);
  ...
  if (player.currentEternityChall == "eterc7")
    ...
    document.getElementById("infPowPerSec").textContent =
      "You are getting " + shortenDimensions(r) + " Infinity Power per second.";
  }
}
...
function getReplSpeed() {
  ...
  if (player.dilation.upgrades.includes("ngmm10"))
    exp += player.dilation.upgrades.length;
  inc = inc + 1;
...
function updateDilation() {
  if (
    ...
    document.getElementById("dilatedGalaxies").textContent = getFullExpansion(
      Math.floor(player.dilation.freeGalaxies)
    );
  }
}
...
function getNewInfReq() {
  ...
  for (var tier = 0; tier < 7; tier++)
    if (!player.infDimensionsUnlocked[tier])
      return { money: reqs[tier], tier: tier + 1 };
...
function infPoints2Display() {
  if (
    ...
    player.infinityUpgrades.length > 0 ||
    getEternitied() > 0 ||
    quantumed
  )
...
function eternityBtnDisplayType() {
  document.getElementById("eternitybtn").style.display =
    ...
    (player.infDimensionsUnlocked[7] || getEternitied() > 24) &&
    (!player.dilation.active || !inQCModifier("ad"))
      ? "inline-block"
      : "none";
}
...
function primaryStatsDisplayResetLayers() {
  ...
  document.getElementById("pastinfs").style.display =
    player.infinitied > 0 || getEternitied() > 0 || quantumed ? "" : "none";
  var showStats =
...
function ECCompletionsDisplay() {
  ...
  document.getElementById("eterc13completed").textContent =
    "Completed " + ECTimesCompleted("eterc13") + " times.";
  document.getElementById("eterc14completed").textContent =
    "Completed " + ECTimesCompleted("eterc14") + " times.";
}

function ECchallengePortionDisplay() {
  let ec12TimeLimit = Math.round(getEC12TimeLimit() * 10) / 100;
  for (var c = 1; c < 15; c++)
    document.getElementById("eterc" + c + "goal").textContent =
      "Goal: " +
      shortenCosts(getECGoal("eterc" + c)) +
      " IP" +
      (c == 12
        ? " in " +
          ec12TimeLimit +
...
function bankedInfinityDisplay() {
  ...
  document.getElementById("infinitiedBank").textContent =
    "You have " +
    getFullExpansion(player.infinitiedBank) +
    " banked infinities.";
  var bankedInfGain = gainBankedInf();
  document.getElementById("bankedInfGain").style.display =
    bankedInfGain > 0 ? "block" : "none";
  document.getElementById("bankedInfGain").textContent =
    "You will gain " +
    getFullExpansion(bankedInfGain) +
    " banked infinities on next Eternity.";
...
function checkIPReqAchieve() {
  ...
  if (ableToGetRid2 && player.infinityPoints.log10() >= 20000)
...
function checkReplicantiBasedReqAchieve() {
  ...
  if (
    player.replicanti.amount.gt(new Decimal(tmp.ngex ? "1e15000" : "1e20000"))
  )
...
function checkMatterAMNDReqAchieve() {
  ...
  if (dilates(player.firstPow, 2) >= 10e30)
...
function checkOtherPreNGp3Achieve() {
  var ableToGetRid2 =
    player.timestudy.studies.length < 1 && player.dilation.active;
  if (tmp.ngp3)
    for (id = 0; id < player.masterystudies.length; id++) {
  ...
  if (player.exdilation !== undefined) {
    let ableToGetRid3 =
      ableToGetRid2 &&
      player.dilation.upgrades.length === 0 &&
...
function ngP3AchieveCheck() {
  ...
  if (
    player.eightBought >= 4e6 &&
    getTotalRG() + player.dilation.freeGalaxies < 1
  )
  ...
  if (
    player.meta.antimatter.e > 332 &&
    player.meta[2].amount.eq(0) &&
    player.meta.resets < 1
  )
  ...
  if (
    player.replicanti.amount.e >= 2e6 &&
    player.dilation.tachyonParticles.eq(0)
  )
    giveAchievement("No dilation means no production.");
  if (
    player.infinityPoints.gte(Decimal.pow(Number.MAX_VALUE, 1000)) &&
    ableToGetRid5
  )
  ...
  if (!player.achievements.includes("ng3p42")) {
    for (d = 2; d < 9; d++) {
      if (
        player[TIER_NAMES[d] + "Amount"].gt(0) ||
        player["infinityDimension" + d].amount.gt(0) ||
        player["timeDimension" + d].amount.gt(0) ||
        player.meta[d].amount.gt(0)
      )
        break;
  ...
  if (
    player.replicanti.amount.e >=
    (player.aarexModifications.ngudpV ? 268435456 : 36e6)
  )
  ...
  if (
    player.options.secrets &&
    player.options.secrets.ghostlyNews &&
    !player.options.newsHidden
  )
    giveAchievement("Two tickers");
  if (!tmp.ngp3l) {
    if (player.ghostify.another && tmp.qu.quarks.gte(1 / 0))
      giveAchievement("Is these another...");
    if (player.ghostify.reference && minUQ.decays >= 2)
      giveAchievement("... reference to EC8?");
    if (
      tmp.hb.bosonicSemipowerment &&
      player.ghostify.ghostlyPhotons.lights[7] >= tmp.leReq / 2
    )
      giveAchievement("Bosonic Semipowerment");
    if (player.ghostify.times >= Math.pow(Number.MAX_VALUE, 1 / 4))
      giveAchievement("The Ghostliest Side");
...
function updateNGpp16Reward() {
  document.getElementById(
    "replicantibulkmodetoggle"
  ).style.display = player.achievements.includes("ngpp16")
    ? "inline-block"
    : "none";
}
  ...
  if (speedrunMilestonesReached > notifyId) {
    $.notify(
      "You have unlocked the " +
        timeDisplayShort(speedrunMilestones[notifyId] * 10) +
        " speedrun milestone! " +
        [
          "You now start with 20,000 eternities on Quantum",
          ...
          "You now start with all Dilation studies and\nnon-rebuyable dilation upgrades up to Meta Dimensions except for the TT gen upgrade",
          "You unlocked the 1st Meta Dimension autobuyer",
          "You unlocked the 2nd Meta Dimension autobuyer",
          "You unlocked the 3rd Meta Dimension autobuyer",
          "You unlocked the 4th Meta Dimension autobuyer",
          "You unlocked the 5th Meta Dimension autobuyer and now keep time studies and the passive TT gen upgrade",
          "You unlocked the 6th Meta Dimension autobuyer",
          "You unlocked the 7th Meta Dimension autobuyer",
          "You unlocked the 8th Meta Dimension autobuyer and\nnow start with all non-rebuyable dilation upgrades",
          "You unlocked the Meta-Dimension Boost autobuyer",
          "You now keep your Mastery Studies",
          "All Meta Dimensions are now available for purchase after Quantum",
          "You now start with " + shortenCosts(1e13) + " eternities",
          "You now start with " +
            shortenCosts(1e25) +
            " meta-antimatter on reset",
          "You can now turn on automatic replicated galaxies without any limits.",
          "The rebuyable dilation upgrade and Meta Dimension autobuyers are now 3x faster",
          "You now start with " +
            shortenCosts(1e100) +
            " dilated time on quantum, and Dilated Time does not reset until quantum",
          "You unlocked the Quantum autobuyer",
          "Replicanti doesn't reset upon Eternity",
          "You unlocked the manual mode for the Eternity autobuyer and got the Electron autobuyer",
          "Your rebuyable dilation upgrade autobuyer can now buy all upgrades possible",
          "You now can buy max meta-dimension boosts and start with 4 meta-dimension boosts",
          "You now gain banked infinities based on your post-crunch infinitied stat",
        ][notifyId] +
        ".",
      "success"
    );
      ...
      "You became a ghost in at most " +
        ...
        [
          "You now start with all Speedrun Milestones and all " +
            shorten(Number.MAX_VALUE) +
            " QK assignation features unlocked, all Paired Challenges completed, all Big Rip upgrades bought, Nanofield is 2x faster until you reach 16 rewards, and you get quarks based on your best MA this quantum",
          "From now on, colored quarks no longer cancel, you keep your gluon upgrades, you can Big Rip as long as you have the electrons, and your study tree persists when completing an Eternity Challenge.",
          "You now keep your Electron upgrades",
          "From now on, going Quantum doesn't reset your Tachyon particles except in a QC, you can unstablize quarks without losing quarks, and you start with 5 of the 1st upgrade of each Branch",
          "From now on, going Quantum doesn't reset your Meta-Dimension Boosts unless in a QC or when undoing the Big Rip",
          "From now on, going Quantum doesn't reset your normal replicants unless in a QC or when undoing the Big Rip",
          "You now start with 10 worker replicants and Neutrinos persist on Ghostify.",
          "You now produce ^0.5 of quarks, ^0.5 of gluons, and 1% of Space Shards gained on Quantum/Big Rip per second.",
          "You now start with 10 of each Emperor Dimension, up to the 2nd tier, " +
            (player.aarexModifications.ngudpV
              ? " and from now on, you start Big Rips with the 3rd row of Eternity upgrades"
              : ""),
          "You now start with 10 of each Emperor Dimension, up to the 4th tier",
          "You now start with 10 of each Emperor Dimension, up to the 6th tier, and the IP multiplier no longer costs IP",
          "You now start with 10 of each Emperor Dimension, up to the 8th tier",
          "You now start with 16 Nanofield rewards",
          "You now start with " +
            shortenCosts(1e25) +
            " quark spins and Branches are 10x faster",
          "You now start with Break Eternity unlocked and all Break Eternity upgrades bought",
          "From now on, you gain 1% of quarks gained on Quantum, and Tachyon particles persist on Quantum and Ghostify except in Big Rips.",
        ][notifyId2] +
        ".",
      "success"
    );
...
function doNGP4UnlockStuff() {
  $.notify(
    "Congratulations! For unlocking Big Rip in a NG+3 save, you unlocked NG+4! Check it out in the mods menu.",
    "success"
  );
  metaSave.ngp4 = true;
  checkForExpertMode();
...
function doGhostifyUnlockStuff() {
  ...
  document.getElementById("welcomeMessage").innerHTML =
    "You are now able to complete the challenge in Big Rip! However, due to the instability of this universe, the only way to go beyond is to become a ghost, allowing you to pass big rip universes and unlock new features, but will remove everything you have up to this point. Therefore, this is the sixth layer of NG+3.";
}
...
function doReachAMGoalStuff(chall) {
  ...
  document.getElementById("welcomeMessage").innerHTML =
    ...
    shorten(Decimal.pow(Number.MAX_VALUE, 1.45)) +
    " meta-antimatter" +
    (tmp.qu.bigRip.active
...
function doQuantumUnlockStuff() {
  ...
  document.getElementById("welcomeMessage").innerHTML =
    ...
    " MA, and completed EC14 for the first time, unlocking Quantum (the 5th layer)! Going quantum will reset everything up to this point, but you will get quarks. This allows you to get gigantic numbers!";
}
...
function doNGP3UnlockStuff() {
  ...
  if (
    ...
    player.meta.bestAntimatter.lt(Decimal.pow(Number.MAX_VALUE, 1.45)) &&
    !tmp.qu.nonMAGoalReached.includes(chall)
  ) {
  ...
  if (
    ...
    doNGP4UnlockStuff();
  if (
    player.eternityPoints.gte("1e1200") &&
    tmp.qu.bigRip.active &&
    !tmp.qu.breakEternity.unlocked
  )
    doBreakEternityUnlockStuff();
  if (
    player.money.gte(Decimal.pow(10, 6e9)) &&
    tmp.qu.bigRip.active &&
...
function passiveIPupdating(diff) {
  ...
  if (player.partInfinityPoint >= 100) {
    player.infinityPoints = player.infinityPoints.plus(
      player.infMult.times(kongIPMult * (player.partInfinityPoint / 10))
    );
...
function changingDecimalSystemUpdating() {
  ...
  if (break_infinity_js) {
    ...
    clearInterval(gameLoopIntervalId);
    alert(
      "You have reached the limit of break_infinity.js, requiring a switch to logarithmica_numerus.js to handle numbers beyond the limit. This will refresh the page, but you cannot change libraries for this save again in the future."
    );
...
function WZBosonsUpdating(diff) {
  ...
  data.speed = Math.max(
    Math.min(wattGained + (data.watt - data.speed) * 2, wattGained),
    data.speed
  );
...
function nanofieldProducingChargeUpdating(diff) {
  ...
  if (toSub.eq(0)) {
    ...
    document.getElementById("produceQuarkCharge").innerHTML =
      "Start producing preon charge.<br>(Preon production is disabled while producing preon charge)";
  } else {
...
function updateNextPreonEnergyThreshold() {
  if (
    tmp.qu.nanofield.energy.gte(tmp.qu.nanofield.powerThreshold) &&
    tmp.qu.nanofield.power < 15
  ) {
    var toAdd = Math.min(
      Math.floor(
        tmp.qu.nanofield.energy.div(tmp.qu.nanofield.powerThreshold).log(4) /
          tmp.ppti +
          1
      ),
      15 - tmp.qu.nanofield.power
    );
    tmp.qu.nanofield.power += toAdd;
    tmp.qu.nanofield.powerThreshold = tmp.qu.nanofield.powerThreshold.times(
      Decimal.pow(4, toAdd * tmp.ppti)
    );
  }
  if (
    tmp.qu.nanofield.energy.gte(tmp.qu.nanofield.powerThreshold) &&
    tmp.qu.nanofield.power > 14
  ) {
    var b = tmp.qu.nanofield.power - 13.5;
    var toAdd = Math.floor(
      Math.sqrt(
        b * b +
          (2 *
            tmp.qu.nanofield.energy
              .div(tmp.qu.nanofield.powerThreshold)
              .log(4)) /
            tmp.ppti
      ) -
        b +
    ...
    tmp.qu.nanofield.power += toAdd;
    tmp.qu.nanofield.powerThreshold = tmp.qu.nanofield.powerThreshold.times(
      Decimal.pow(4, (0.5 * toAdd * toAdd + b * toAdd) * tmp.ppti)
    );
    if (tmp.qu.nanofield.power > 124)
      tmp.qu.nanofield.powerThreshold = tmp.qu.nanofield.powerThreshold.times(
        Decimal.pow(4, tmp.qu.nanofield.power - 124)
      );
    if (tmp.qu.nanofield.power > 149)
      tmp.qu.nanofield.powerThreshold = tmp.qu.nanofield.powerThreshold.times(
        Decimal.pow(1.1, -100 + Math.pow(tmp.qu.nanofield.power - 140, 2))
      );
      ...
      if (!tmp.qu.nanofield.apgWoke && tmp.qu.nanofield.rewards >= tmp.apgw) {
        ...
        $.notify(
          "You reached " +
            ...
            " rewards... The Anti-Preontius has woken up and has taken over the Nanoverse! Be careful!"
        );
        showTab("quantumtab");
...
function replicantEggonUpdating(diff) {
  var newBabies = tmp.twr
    ...
    .times(getSpinToReplicantiSpeed())
    .times(diff / 200);
...
function replicantBabiesGrowingUpUpdating(diff) {
  if (tmp.qu.replicants.babies.gt(0) && tmp.tra.gt(0)) {
    tmp.qu.replicants.ageProgress = tmp.qu.replicants.ageProgress
      .add(
        tmp.tra.times(
          diff / (player.achievements.includes("ng3p35") ? 400 : 4e3)
        )
      )
...
function otherDimsUpdating(diff) {
  ...
  if (player.currentEternityChall == "eterc7")
    player.infinityDimension8.amount = player.infinityDimension8.amount.plus(
      getTimeDimensionProduction(1).times(diff)
    );
  else {
...
function nonERFreeTickUpdating() {
  ...
  ).times(player.aarexModifications.ngmX > 3 ? 0.01 : 1);
  document.getElementById("totaltickgained").textContent =
    "You've gained " +
    getFullExpansion(player.totalTickGained) +
    " tickspeed upgrades.";
    ...
    if (player.money.gte(Number.MAX_VALUE)) {
      ...
      if (IPminpeak.l > 1e9)
        ...
        var notationPart =
          ...
          player.options.notation == "Spazzy";
        var IPminpart = "";
        if (IPminpeak.l < 1e5 && notationPart)
          IPminpart =
            "<br>" +
        ...
        document.getElementById("postInfinityButton").innerHTML =
          "<b>" +
          (IPminpeak.l > 3e7 && notationPart ? "Gain " : "Big Crunch for ") +
...
function getReplicantiInterval() {
  ...
  if (player.aarexModifications.ngexV) interval *= 0.8;
  ...
  if (player.exdilation != undefined)
    interval = interval.div(getBlackholePowerEffect().pow(1 / 3));
  if (
    player.dilation.upgrades.includes("ngpp1") &&
    player.aarexModifications.nguspV &&
    !player.aarexModifications.nguepV
  ...
  if (player.dilation.upgrades.includes("ngmm9"))
    interval = interval.div(getDil72Mult());
  if (tmp.ngp3)
    if (player.masterystudies.includes("t332"))
      interval = interval.div(getMTSMult(332));
  return interval;
}
    ...
    x = player.boughtDims
      ? Math.pow(
          player.achievements.includes("r107")
            ? Math.max(player.replicanti.amount.log(2) / 1024, 1)
            : 1,
          -0.25
        ) * x.toNumber()
      : Decimal.pow(
          tmp.rep.speeds.inc,
          Math.max(player.replicanti.amount.log10() - tmp.rep.speeds.exp, 0) /
            tmp.rep.speeds.exp
        ).times(x);
  return x;
}
...
function notContinuousReplicantiUpdating() {
  ...
  if (interval <= replicantiTicks && player.replicanti.unl) {
    if (player.replicanti.amount.lte(100)) runRandomReplicanti(chance);
    //chance should be a decimal
      ...
      if (!player.timestudy.studies.includes(192))
        player.replicanti.amount = player.replicanti.amount.min(
          getReplicantiLimit()
        );
    }
...
function continuousReplicantiUpdating(diff) {
  if (
    player.timestudy.studies.includes(192) &&
    tmp.rep.est.toNumber() > 0 &&
    tmp.rep.est.toNumber() < 1 / 0
  )
    player.replicanti.amount = Decimal.pow(
      Math.E,
      tmp.rep.ln +
        Math.log(
          ((diff * tmp.rep.est) / 10) *
            (Math.log10(tmp.rep.speeds.inc) / tmp.rep.speeds.exp) +
            1
        ) /
          (Math.log10(tmp.rep.speeds.inc) / tmp.rep.speeds.exp)
    );
    ...
    player.replicanti.amount = Decimal.pow(
      ...
      tmp.rep.ln +
        tmp.rep.est
          .times(
            (diff * Math.log10(tmp.rep.speeds.inc)) / tmp.rep.speeds.exp / 10
          )
          .add(1)
          .log(Math.E) /
          (Math.log10(tmp.rep.speeds.inc) / tmp.rep.speeds.exp)
    ...
    player.replicanti.amount = Decimal.pow(
      Math.E,
      tmp.rep.ln + (diff * tmp.rep.est) / 10
    ).min(getReplicantiLimit());
  replicantiTicks = 0;
}
  ...
  document.getElementById("quantumbtnFlavor").textContent =
    ((tmp.qu !== undefined
      ...
      ? ((tmp.ngp3 ? tmp.qu.bigRip.active : false)
          ...
          ? "Paired challenges are"
          : "My challenging skills are") + " not powerful enough... "
...
function doGhostifyButtonDisplayUpdating(diff) {
  ...
  document.getElementById("ghostifybtnFlavor").textContent =
    ghostifyGains.length > 1
      ? ""
      : (ghostifyGains.length ? "" : "This broken universe will be done... ") +
...
function currentChallengeProgress() {
  ...
  document
    ...
    .setAttribute("ach-tooltip", "Percentage to the challenge goal");
}
...
function currentEChallengeProgress() {
  ...
  document
    .getElementById("progresspercent")
    .setAttribute("ach-tooltip", "Percentage to the Eternity Challenge goal");
}
...
function ECRewardDisplayUpdating() {
  ...
  document.getElementById("ec13reward").textContent =
    "Reward: Increase the exponent of meta-antimatter effect. (" +
    (getECReward(13) + 9) +
    "x)";
  ...
  if (player.ghostify.milestones > 7) {
    ...
    for (var u = 1; u <= getMaxBigRipUpgrades(); u++) {
      document.getElementById(
        ...
        : tmp.qu.bigRip.spaceShards.lt(bigRipUpgCosts[u])
        ? "gluonupgrade unavailablebtn"
        : "gluonupgrade bigrip";
      document.getElementById(
        "bigripupg" + u + "cost"
      ).textContent = shortenDimensions(new Decimal(bigRipUpgCosts[u]));
    }
  }
...
function TTpassiveGain(diff) {
  if (player.dilation.upgrades.includes(10)) {
    ...
    if (div < 3600)
      player.timestudy.theorem += Math.min(diff * 9, 3600 - div) * speed;
    if (player.timestudy.theorem > 1e200) player.timestudy.theorem = 1e200; // hardcapped at 1e200
  }
}
  ...
  if (storage.dt) {
    ...
    if (tmp.qu.replicants.amount.gt(storage.nr))
      popupString +=
        ...
        " orders of magnitude";
    if (tmp.bl.am.gt(storage.ma))
      popupString +=
        ",<br> Bosonic Antimatter increased " +
        shortenMoney(tmp.bl.am.log10() - Decimal.max(storage.bAm, 1).log10()) +
        " orders of magnitude";
  }
  if (
...
function updatePowers() {
  totalMult = Math.pow(
    ...
    player.galacticSacrifice ? 2 : 0.5
  );
  currentMult = Math.pow(
    player.money.e + 1,
    player.galacticSacrifice ? 2 : 0.5
  );
  ...
  achievementMult = Math.max(
    Math.pow(
      player.achievements.length -
        (player.galacticSacrifice ? 10 : 30) -
        getSecretAchAmount(),
      player.galacticSacrifice ? 5 : 3
    ) / 40,
    1
  );
...
function switchDecimalMode() {
  if (
    confirm(
      "You will change this option to " +
        (player.aarexModifications.breakInfinity
          ? "logarithmica_numerus_lite"
          : "break_infinity.min") +
        ".js. Switching number libraries requires a page reload. Are you sure you want to do that?"
    )
  ) {
    player.aarexModifications.breakInfinity = !player.aarexModifications
      .breakInfinity;
    if (
      ...
      !player.aarexModifications.performanceTicks &&
      confirm(
        "WARNING: Your game may lag when using break_infinity.js. Do you want to turn on Performance Ticks? You can change your preference at anytime in the Options menu."
      )
    )
